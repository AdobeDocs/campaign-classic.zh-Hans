...
* 修复了在将 **Interaction** 模块与 [Power Booster](../../installation/using/power-booster-and-power-cluster.md) 选项结合使用时可能导致无法正确管理产品建议的问题。(NEO-39263)
...
* 已将缺少的数据库索引添加到以下架构中，以防止出现数据库更新问题：xtk:rights、nms:dlvExclusion、nms:seedMember、nms:trackingUrl

**补丁程序**

* 修复了将产品建议链接到投放时，导致“热门点击”报表无法工作的问题。(NEO-26295)
...
* 修复了将中间服务器设置为跟踪服务器和营销服务器之间的中继时出现的跟踪日志重复数据删除问题。(NEO-36285)
...
<p>有关更多信息，请参阅<a href="../../workflow/using/change-data-source.md">详细文档</a>。</p>
</td>
</tr>
</tbody>
</table>
...
* 现在，仅管理员用户可访问返回完整数据库连接详细信息的&#x200B;**xtk:session#GetCnxInfo** API方法。 (NEO-27779)
...
* 从 Campaign 21.1 版本开始，弃用 Adobe Analytics Data Connector。如果使用此连接器，则需要使用新连接器 Adobe Analytics Connector 相应地调整实施。
* 现已弃用对 Debian 8 的支持。
...
* **计费 (billing)** 技术工作流现在包括原来由&#x200B;**活跃计费轮廓数** (billingActiveContactCount) 工作流执行的任务，后者已被删除。工作流每月发送的电子邮件报表现在将提供关于实例上活跃轮廓数的信息。[阅读更多](../../workflow/using/about-technical-workflows.md)。
...
* 修复了使用Web应用程序时导致跟踪标记在&#x200B;**nms:trackingURL**&#x200B;架构中生成不正确的主键的问题。 (NEO-27931)
...
* 修复了在搜索与产品建议相关的建议数据时可能导致性能不良的问题。(NEO-27554)
* 修复了当 IMS 服务被激活但未响应时导致 Web 进程没有响应的问题。
* 修复了由于特定加入机制导致投放个性化失败，因此导致无法发送包含一组校样的投放的问题。(NEO-14391)
...
* 修复了即使产品建议的结束日期已过，仍可能允许显示该产品建议信息的问题。Campaign Classic 现在会考虑结束日期的整个时间戳，而不是只考虑日期。(NEO-27590)
