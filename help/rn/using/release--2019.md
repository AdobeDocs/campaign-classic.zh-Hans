...
* 出于安全原因，在使用时，您无法再插入任意命令 **[!UICONTROL Pre-process the file]** 中的选项 **[!UICONTROL Data loading (file)]** 工作流活动。 现在有一个下拉列表，允许您从3个选项中进行选择： **[!UICONTROL None]**， **[!UICONTROL Decompression]** (zcat)或 **[!UICONTROL Decrypt]** (gpg)。 已添加XtkSecurity_Disable_Preproc安全标志。 对于新客户，此选项将设置为0。 对于现有客户，升级后此选项将设置为1，以保留以前的行为。 请参阅此 [部分](../../workflow/using/data-loading--file-.md).
