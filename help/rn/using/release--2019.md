...
* 出于安全原因，在使用 **[!UICONTROL Pre-process the file]** 选项 **[!UICONTROL Data loading (file)]** 工作流活动。 现在提供了一个下拉列表，允许您从3个选项中进行选择： **[!UICONTROL None]**, **[!UICONTROL Decompression]** (zcat)或 **[!UICONTROL Decrypt]** (gpg)。 已添加XtkSecurity_Disable_Preproc安全标志。 对于新客户，此选项将设置为0。 对于现有客户，为了保持先前的行为，升级后会将此选项设置为1。 请参阅此[章节](../../workflow/using/data-loading--file-.md)。
