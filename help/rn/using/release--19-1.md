...
* 修复了在数据管理活动中进行数据提取时，在默认数据库而不是联合数据访问数据库上执行的SQL语句“SELECT COUNT”的问题。
...
* 修复UUID（唯一通用标识符）问题会导致扩充活动在Hive联合数据访问中失败。
...
* 修复了文件传输 **活动的问题** ，该问题导致基于SFTP密钥的身份验证无法在Debian 9上运行。 (NEO-23183)
...
* 改进了LibreOffice管理，以避免电子邮件预览中。odt文件失败。
* 改进了Apache连接的管理，以避免Web服务上的延迟。
* 改进了版本标签（7位）在“关于”菜单中 **的显示** 。
* 修复了列表管理中的回归问题，以防止发布优惠。
* 修复了导致清除工作流崩溃的回归。
* 修复了清除工作流日志中的次要回归问题。
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>要提高管理员用户的工作效率，请通过监控存储、将IP地址列入白名单以及为每个实例安装SSH密钥来管理SFTP服务器的设置。 请注意，控制面板仅适用于今天起在AWS上托管的客户(<a href="https://experiencecloud.adobe.com/campaign/controlpanel/">今天通过Experience Cloud登录</a>)。</p> <p>有关详细信息，请参 <a href="https://docs.adobe.com/content/help/zh-Hans/control-panel/using/control-panel-home.html">阅详细文档</a><a href="https://docs.adobe.com/content/help/en/campaign-learn/campaign-classic-tutorials/administrating/control-panel-acc/control-panel-overview.html">和操作方法视频</a>。 </p><p>注意：访问控制面板不需要升级到最新的活动版本。</p> </td> 
    ...
    <tr> 
   <td> 审核跟踪<br /> </td> 
   <td> <p>作为管理员，通过监视和管理在Adobe Campaign经典实例中所做的更改来提高工作效率。 审核跟踪将记录对源模式、工作流和选项执行的操作。 您可以快速查看是否已创建、修改或删除元素。</p><p>有关详细信息，请参阅详 <a href="../../production/using/audit-trail.md">细文档</a><a href="https://docs.adobe.com/content/help/en/campaign-learn/campaign-classic-tutorials/monitoring/audit-trail.html">和操作方法视频</a>。</p></td> 
  </tr> 
  <tr> 
   <td> Guardrail、强健性和可伸缩性<br /> </td> 
   <td> 在Campaign Classic中增加了一系列改进。 下面列出了增强的安全性、强健性和可伸缩性。<br /> </td> 
  </tr> 
  <tr> 
   <td> 兼容性矩阵更新<br /> </td> 
   <td> 有了这个新版本，Adobe Campaign现在支持以下数据库系统。 请参阅兼 <a href="https://helpx.adobe.com/campaign/kb/compatibility-matrix.html">容性表</a>。<br /> 
    <ul> 
...
* 联合数据访问HTTP v2:基于HTTP协议的联合数据访问在Hybrid部署中得到广泛应用，尤其是用于广泛的Log检索和投放准备。 增强了健壮性，以避免在检索或推送数据时出现网络问题和可能的错误。 这要求连接两端的内部版本保持最新，否则仍将使用旧协议。
...
* 操作符操作的护栏：已实施若干保护栏以防止运营商执行可能影响平台完整性的操作。 内置模式不再通过界面删除。 此外，非管理员用户不再可以编辑工作流源XML。
...
* 现在，在分析反编译投放期间，如果发布模式在部署向导中设置为， **[!UICONTROL None]** 则会记录一个错误并停止分析:“发布模式设置为‘无’:无法嵌入图像。 功能电话上不显示图像。” (NEO-12208)
* 针对事务性消息传递改进了广播管理。 当广播从执行实例同步到控制实例时，@lastModified字段将更新为系统的当前日期。 MC_Update_BlLastModified选项已添加用于控制实例。 True表示当前日期将用于控制实例（默认行为）。 False表示我们使用执行实例广播的@lastModified日期。 (NEO-12579)
...
* 修复了导致用户活动与打开的报告指示器的跟踪报告之间出现投放差异的问题。 (NEO-11742)
* 修复了IMS登录的问题。
* 修复了从库添加图像时可能导致投放中缺失图像的问题。 (NEO-11900)
* 修复了在直邮优惠中提取投放详细信息时可能发生的问题。 (NEO-11700)
* 修复了可能影响SMS事务性消息性能的问题。 (NEO-9812)
* 修复了在投放的主目标中使用外部文件选项“定义”时可能发生的控制台崩溃问题。 (NEO-12349)
...
* 修复了在优惠管理中预览出站XML或CSV文件以进行直邮投放时的问题。 (NEO-11290)
...
* 修复了在活动后添 **[!UICONTROL Alert]** 加扩充活动时导致工作流查询失败的问题。 (NEO-12132)
* 修复了Oracle设置中管线（触发器）偏移未从数据库成功检索而导致重复的问题。 (NEO-12121)
* 修复了在使用Analytics集成时，导致透视表中显示错误的问题(NEO-12103)
* 修复了描述性分析报表的问题。 (NEO-11414)
* 修复了CRM连接器在远程表名称中包含带下划线的字段时的问题。
* 修复了可能导致假设验证报告中货币符号显示的问题。 (NEO-11634)
