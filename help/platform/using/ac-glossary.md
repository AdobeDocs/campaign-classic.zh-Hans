...
以下是Adobe Campaign中关键术语和概念的定义，以及相关文档的链接。 单击术语以显示其定义。
...
进一步了解 [本地审批活动](../../workflow/using/local-approval.md).
+++

+++**受众**

受众是基于规则和属性且符合过滤器定义标准的生成配置文件集。
...
审核记录可实时捕获在Adobe Campaign实例中发生的操作和事件的全面列表。 它提供了一种访问数据历史的自助方式，可帮助回答以下问题：您的工作流发生了什么情况、上次更新这些工作流的人或您的用户在实例中做了什么。

详细了解 [审核记录](../../production/using/audit-trail.md).
+++
...
在促销活动交互的上下文中，批模式允许选件引擎为一组联系人选择最佳选件（或多个选件）。 资格/优先级规则应用于集的所有联系人。
...
Campaign是一个用于协调、定义和执行营销活动的界面。 营销策划可以将一个或多个工作流、投放、文档和其他相关数据点包含到一个易于使用的界面中。

详细了解 [营销活动](../../campaign/using/designing-marketing-campaigns.md).
+++
...
信道是用来发送通信的媒体。 Adobe Campaign中的内置渠道包括电子邮件、短信、直邮、推送通知、LINE和Twitter。 可以针对非标准渠道要求实施自定义渠道。
...
Campaign客户端控制台是一个富客户端，可用于连接到Campaign应用程序服务器。 客户端控制台可执行文件(.exe)应用程序安装在具有Microsoft Windows操作系统的计算机上。 Campaign客户端控制台集中了所有功能和设置。
...
内容批准是指让单独的操作员或操作员组在发送投放内容之前批准该内容的过程。

详细了解 [内容审批](../../campaign/using/marketing-campaign-approval.md).
+++

+++**对照组**

使用控制组通过排除部分控制组受众来衡量促销活动的影响。 操作员可以将收到消息的目标群体的行为与未作为目标的联系人的行为进行比较。 根据发送日志，操作员还可以在未来的营销活动中以控制组为目标。

详细了解 [对照组](../../campaign/using/marketing-campaign-target.md#defining-a-control-group).
+++

+++**控制面板**

该控制面板允许Adobe Campaign产品管理员管理每个实例的设置并跟踪使用情况，从而帮助他们提高工作效率。 其直观的界面让他们能够轻松监控关键资产的使用情况，并执行管理任务，如IP地址允许列表添加、 SFTP存储监控、密钥管理等。
...
Adobe Campaign附带预定义的数据模型，其中数据类型通过安装各种包来定义。 运算符可以通过扩展资源以添加自定义字段或自定义表（如交易表或产品表）来扩充提供的数据模型。
...
通过可投放性，您可以衡量营销活动在到达收件人的收件箱时不会出现退回或标记为垃圾邮件的情况。 更准确地说，电子邮件可投放性是指一组特征，这些特征可决定消息通过个人电子邮件地址在短时间内到达其目标的能力，以及在内容和格式方面达到预期质量的能力。

详细了解 [可投放性](../../delivery/using/about-deliverability.md).
+++

+++**投放**

投放是通过特定渠道（电子邮件、短信、推送通知等）发送给受众的特定营销通信项目。 在营销术语中又称为“触控”。
...
投放分析是投放的准备工作。 此过程会将内容与收件人用户档案数据相结合，生成收件人将收到的个性化电子邮件。 投放分析逻辑可以根据定义的逻辑，从目标中排除收件人或完全停止投放。 此过程还包括动态内容逻辑的评估，以及插入特定于单个收件人配置文件的选件。
...
投放概要是一组结构化元素（文档、商店、促销优惠券等） 由公司和为特定营销活动创建。 它在直邮投放的上下文中使用。
...
描述性分析是一种上下文相关的报告工具，可用于检查工作流工作表中的数据、文件夹中选择的数据，或者对所选投放的目标和排除项进行深入分析。
...
分布式营销附加组件为Campaign操作员提供了一个协作工作区，用于在中央实体（总部、营销部门等）之间实施营销活动 实施协作营销活动。此合作基于共享工作区，称为 **Campaign包列表**，向本地实体提供集中创建的营销活动模板和实例。
...
值分布是一种工具，它显示当前存在于数据库中的模式属性的值分布。 这有助于您确定哪些值可用、其计数和百分比，并在创建查询或表达式时避免出现值的大写和拼写问题。
...
子域配置允许您配置域的子部分（技术上称为“DNS区域”）以用于Adobe Campaign。
...
资格规则是应用于环境、类别或优惠的限制，涉及有效期、目标和权重。 它们允许操作员确保选件符合定向联系人。 在优惠环境中，资格规则包括应用于优惠和要定位的收件人的呈现规则。 在类别中，资格规则允许操作员及时限制类别的有效性、定义应用程序主题并确定要定位的收件人。 它们还可以为给定时间段定义乘数权重。 这允许操作员共享其他类别中选件的规则，从而简化其管理。 在优惠中，资格规则允许操作员及时限制优惠的有效性并确定要定位的收件人。
...
电子邮件密件抄送功能以EML格式发送相应投放电子邮件的精确副本，该副本将保存到专用的密件抄送电子邮件地址，发件人可以在外部系统中处理和存档电子邮件。
...
扩充活动是一种高级工作流活动，它允许操作员扩充将在工作流中处理的工作表数据。 此活动通常用在定向活动或导入文件之后，以及使用定向数据的活动之前。 “增强”功能可以转换集客过渡数据并配置活动，以使用增强的数据完成输出过渡。 它允许操作员组合来自多个数据集的数据，或创建指向临时资源的链接。
...
详细了解 [明细列表](../../platform/using/managing-enumerations.md).
+++
...
详细了解 [资源管理器视图](../../platform/using/adobe-campaign-explorer.md).
+++

+++**外部帐户**

外部帐户是产品连接到其他环境和技术的入口和出口点。 外部帐户定义产品用来向其他源发送数据或从其他源接收数据的连接参数。 典型的外部帐户类型包括SFTP站点的连接、支持发送SMS的电信、用于处理退回的邮箱或与外部数据库的连接。
...
疲劳管理可帮助您控制消息的频率和数量，以避免过度招徕收件人，通常使用类型规则来应用疲劳管理。
...
联合数据访问支持扩展客户端数据模型以包含第三方数据库。 它将自动检测目标表的结构，并使用来自SQL源的数据。 您可以访问外部数据，而无需更改Adobe Campaign数据的结构。
...
文件提取审批是一个流程，该流程要求单独的操作员或操作员组在提取文件发送到外部供应商（如直邮投放）之前审批其内容和配置。

详细了解 [文件提取审批](../../delivery/using/validating.md).
+++

+++**过滤维度**

筛选维度是包含数据或属性的架构，查询使用这些数据或属性来筛选所需的行。 筛选维度架构必须直接链接到定义的定位维度，以允许Adobe Campaign跨数据库联接并返回响应行。
...
文件夹是保存特定数据类型的数据库记录的Explorer视图项。 用作组织元素的Generic文件夹类型例外，它本身不包含任何数据，只包含其他文件夹。
...
Forms定义特定架构类型的界面表示。 Forms是一种用于在产品中轻松创建和编辑数据元素的方法，例如收件人、投放和营销活动。 Adobe Campaign中的所有界面元素都是使用Forms在产品本身中创建的。 请注意，表单是可选的，并非所有架构都有表单。
...
Campaign热图是一个显示24小时期间的工作流执行信息的表。 它以小时和5分钟为间隔显示整个期间的工作流分布。 热图用于评估服务器负载并确定占用资源最多的工作流活动。
...
混合部署是按需服务和部署的内部部署软件的组合，可共同发挥作用。
...
* **显式**：联系人登录渠道界面后进行标识。
* **隐式**：联系人已由Cookie（永久或会话）识别。 它可以作为匿名联系人或识别联系人处理。
* **匿名**：无法识别联系人。
...
入站交互是指在渠道（例如网络、呼叫中心或移动设备）中联系人操作生成的来话呼叫之后的交互。 此类交互通常在单一模式下处理（即每个收件人）。
...
收件箱渲染是生成电子邮件预览，它确保消息以最佳方式在各种Web客户端、Web邮件和设备上向收件人显示。 Adobe Campaign利用Litmus，允许电子邮件内容创建者在70多个电子邮件渲染器中预览其消息内容，例如Gmail收件箱或Apple Mail客户端。
...
作业由向导系统管理，简化了数据导入和导出到产品的过程。 作业使用模板系统以简化和保持一致性，并且可以定义为按计划执行。
...
列表是静态数据集。 列表是从其他来源(Audience Manager、Experience Platform、数据库等)导入Campaign的受众或区段，并在界面中被视为导入的列表。
...
本地缓存是存储在操作员计算机上的本地信息。 控制台使用缓存的信息来减少服务器所需的流量并提高性能。 定期清除本地缓存（在“文件”菜单上）可更新存储的信息，并提高性能和稳定性。
...
此 **营销资源管理(MRM)** 通过Adobe Campaign中的模块，您可以对涉及的任务、预算和营销资源进行完整管理和实时跟踪，从而在协作模式下控制营销活动。 Adobe Campaign操作员可以通过完整的验证流程和适当的跟踪工具（报告、跟踪批准、通知、论坛等），协调其行动并批准其所有阶段的进度。
...
用于定义操作员组访问权限和权限（角色）的粒度数据库访问权限。 命名权限会在产品安装期间通过导入定义工具特定功能的各种包来填充。 可创建自定义命名权限以支持客户端业务要求。
...
命名空间是一个分区，可将客户数据类型与数据模型中Adobe Campaign的本机数据类型分隔开。 还用于促进定义从一个实例迁移到另一个实例，例如将架构或模板从开发实例移动到生产实例。
...
导航树是Adobe Campaign资源管理器视图中的主导航。 导航树的工作方式与文件浏览器类似（例如Windows资源管理器）。 文件夹可能包含子文件夹。 选择节点将显示与该节点对应的视图。 显示的视图是与架构关联的列表和用于编辑所选行的输入表单。 您可以自定义导航树并设置文件夹权限。
...
在营销策划、项目或计划中，操作员可以陈述目标列表。 这些是要实现的量化值。 在营销活动、项目或计划结束时， MRM模块允许操作员在专用报告中比较目标和结果。
...
优惠目录是在Adobe Campaign中定义的一组优惠，可在交互期间选择。 目录按层次进行组织，每个节点对应于类别。
...
优惠联系人是指来自入站交互的联系人。 在引擎调用处理期间，联系人与定向维度相关联。 未识别的匿名联系人归因于访客定向维度。 有两种类型的联系人，即已识别和匿名：
...
选件 **设计环境** 是操作员可在其中创建选件、定义分类规则并选择选件定位的架构的环境。 用于存储生成的选件建议的表也由环境定义。 默认情况下，交互加载项附带 **设计** 环境和 **实时** 链接到它的环境。 这两个环境都已预配置为定位内置的收件人表。

详细了解 [优惠设计环境](../../interaction/using/fundamental-principles.md).
+++
...
优惠引擎选择将在环境中显示的优惠（符合条件的优惠）。 套利原则根据类别和优惠中定义的标准，按优先级对优惠进行排名。
...
详细了解 [优惠环境](../../interaction/using/fundamental-principles.md).
+++
...
选件实时环境已链接到营销活动 **设计环境**. 它包含只读选件，其内容和资格已通过 **设计环境**. 可以选择它们以在网站上显示或插入到出站消息中。

详细了解 [提供实时环境](../../interaction/using/fundamental-principles.md).
+++
...
详细了解 [优惠呈现规则](../../interaction/using/managing-offer-presentation.md#presentation-rules-overview).
+++
...
这是选件在其文件夹中显示的预览。 可从优惠预览选项卡或联系人配置文件访问该设置。

详细了解 [优惠预览](../../interaction/using/creating-an-offer.md#previewing-the-offer).
+++
...
优惠建议是操作的结果，该操作包括将优惠提供给给定优惠空间中的联系人，例如网站上的横幅、电子邮件或短信内容。 此结果存储在优惠建议表中，该表定义了优惠、收件人和时间戳，提供了收件人已收到的所有优惠的记录。

详细了解 [优惠建议](../../interaction/using/creating-offer-spaces.md#offer-proposition-statuses).
+++
...
优惠呈现是渠道用于显示优惠的信息。 优惠表示可以从其上表示优惠的空间的呈现函数构造或者直接输入到界面(例如，在HTML块中)中。 优惠可以用空格表示。
...
通过选件模拟，操作员可以测试定义范围（投放日期、目标区段、选件数量、主题等）内的选件分发 才实际发送选件。 它可用于调整优惠优先级和资格规则，以最大限度地提高优惠效率。
...
详细了解 [优惠空间](../../interaction/using/creating-offer-spaces.md).
+++
...
优惠主题是在类别中定义的关键字，允许操作员在显示优惠时对其进行过滤。 主题允许从目录结构非分层选择选件。
...
优惠权重基于精确定义优惠相关性的公式，以便允许引擎选择最相关的优惠。 权重在优惠中定义，乘数在类别中定义。 符合条件的优惠会在权重降序时考虑在内。

详细了解 [优惠权重](../../interaction/using/creating-an-offer.md#offer-weight).
+++
...
操作员组允许您管理Campaign操作员的角色。 您可以定义赋予权限的运算符组，然后将运算符与一个或多个组相关联。 这使您能够重用权限并使操作员配置文件更加一致。 它还便于用户档案的管理和维护。

详细了解 [操作员组](../../platform/using/access-management-groups.md).
+++

+++**选项**

选项是用于定义Campaign实例设置的平台级别变量。 选项可以在平台级别定义时间范围（例如数据库清理工作流）或其他全局定义。
...
出站交互是指从联系人列表（用于投放电子邮件、直邮等）对交互引擎的调用。 相同的规则和流程适用于每个联系人。 此类交互通常在批处理模式下处理。

详细了解 [出站交互](../../interaction/using/about-outbound-channels.md).
+++

+++**资源包导出/导入**

资源包导出是一个操作，它包含生成包含对象定义的XML文件。 包用于将功能和定义从一个实例迁移到另一个实例。 它们还用于向备份和源控制系统添加关键产品定义。

详细了解 [资源包导出/导入](../../platform/using/working-with-data-packages.md).
+++

+++**面板**

工作流面板显示可添加到工作流的可用活动。 此组件以选项卡式格式显示，其中工作流活动按其使用进行逻辑分组。 面板上可用的活动由已安装在Campaign实例中的加载项以及显示工作流的上下文决定。
...
性能监视信息显示在“监视”选项卡上。 它显示底层系统的度量，如内存和CPU使用率、SMTP服务器统计信息、服务器进程以及其他相关信息。
...
Adobe Campaign提供了可插入到投放中的内置个性化块。 它们是动态的、个性化的，并包含特定渲染。 例如，您可以添加徽标、问候语消息或指向镜像页面的链接。 默认情况下，可以使用多个个性化块。 您还可以定义自定义个性化块，以便优化投放个性化。 在投放的分析阶段，实际数据会插入到每个生成的消息中。
...
计划是一种文件夹类型，用于按日历组织营销活动。 资源管理器视图中的计划文件夹定义基于时间的单位，如年、季度或月。 Plan文件夹可以嵌套，并且可以包含其他Plan文件夹、Program文件夹或Campaigns。
...
预定义过滤器是已保存以供重复使用的查询。 使用预定义过滤器可以提高生产效率（因为它们只创建一次）、帮助建立一致性（因为所有营销人员都可以使用它们）并降低营销人员所需的技能，因为他们可以使用他们自己可能无法创建的代码或逻辑。
...
主键是数据库表中每个记录的唯一标识符。 一个表必须至少有一个键。 作为规则，键在架构的主元素和索引之后声明。 主键不能是复合键（包括多个字段）。
...
详细了解 [配置文件](../../platform/using/about-profiles.md).
+++
...
在Adobe Campaign中，收件人是用于发送投放（电子邮件、短信等）的默认用户档案 给您的客户。 通过存储在数据库中的收件人数据，可筛选目标并添加个性化数据。 通常，这些信息包括个人、联系人、人口统计和事务信息，但也可以是支持营销和分析的任何类型的信息。
...
渲染函数在选件空间中定义。 它用于根据优惠中定义的属性构建其优惠表示法。 呈现功能有三种模式：HTML、XML和文本。

详细了解 [渲染函数](../../interaction/using/creating-offer-spaces.md).
+++
...
详细了解 [架构](../../configuration/using/about-schema-reference.md).
+++
...
详细了解 [架构扩展](../../configuration/using/extending-a-schema.md).
+++

+++**种子地址**

种子地址用于定位不符合既定目标标准的收件人。这样，超出投放范围的收件人可以像任何其他目标收件人一样接收投放。 它们会添加到消息的受众，以检测收件人数据库是否用于任何欺诈行为，或确保投放。
...
Adobe Campaign允许您创建和管理新闻稿或产品更新等信息服务，并管理对这些服务的订阅。 多个服务可以并行定义。
...
在控制面板中，您可以与连接到您有权访问的 Campaign 实例的所有 SFTP 服务器进行交互。控制面板允许您对SFTP服务器执行操作，如监视存储容量、管理IP地址允许列表和管理公共SSH密钥。
...
订阅服务工作流活动允许您为过渡中指定的群体创建或删除对信息服务的订阅。
...
目标批准是指在可以发送投放之前（在分析阶段生成目标之后）由单独的操作员或操作员组批准投放的最终目标的过程。

详细了解 [目标审批](../../workflow/using/local-approval.md).
+++

+++**目标数据**

目标数据是存储在工作流的工作表（过渡）中的数据。 此数据可在投放内部使用，用于对投放内容进行个性化或定义投放动态元素的逻辑。
...
定位活动是特定于定位、处理群体数据和筛选活动的工作流活动。 它们允许运算符通过定义集并使用交集、并集或排除操作拆分或组合这些集来构建一个或多个目标。
...
定位维度是由查询或其他工作流活动生成（返回）的数据类型。 请注意，无论使用什么查询获取响应数据库行，Adobe Campaign都只返回响应数据库行的主键。

详细了解 [定位维度](../../workflow/using/targeting-data.md).
+++
...
任务工作流活动将人工操作纳入工作流的逻辑中。 您可以指定两种方案：第一种是任务已完成的情况，第二种是任务未完成的情况。 典型用例用于将离线操作合并到营销策划中，或者用于自定义操作，例如批准。
...
模板是用于创建对象的设计元素。 它包含对象设置和（可选）对象的内容。 模板系统用于创建投放、营销策划、工作流和Adobe Campaign的许多其他元素。 可用的工厂模板由安装的软件包定义。 然后，可根据Campaign操作员的需要复制和自定义模板。
+++
...
跟踪技术工作流会在发送投放并激活跟踪后检索跟踪数据。 此数据可在投放的Tracking选项卡中找到。 您可以查找有关电子邮件或收件人收到的消息中其他交互的打开和点击次数的信息。
...
事务性消息传递是一个Campaign模块，用于管理从外部信息系统发送的事件生成的自定义触发器通知。 事务型消息是由诸如网站之类的提供商实时发送的单个且唯一的通信。 此消息尤其符合预期，因为它包含收件人要检查或确认的重要信息。

详细了解 [事务性消息传递](../../message-center/using/about-transactional-messaging.md).
+++

&lt;!-------在这里很有用??----->
+++**已触发营销活动**

触发的营销活动是在工作流中收到API请求时执行的营销活动。 API调用由启动工作流执行的工作流中的信号活动使用。
...
分类是应用于投放分析阶段的一组分类规则。 活动类型可以包含多个类型规则，但投放只能引用一个类型。

详细了解 [类型](../../campaign-opt/using/about-campaign-typologies.md#typologies).
+++
...
详细了解 [类型规则](../../campaign-opt/using/about-campaign-typologies.md#typology-rules).
+++
...
详细了解 [单一模式](../../interaction/using/about-inbound-channels.md).
+++
...
Web应用程序是由Campaign实例托管的动态和交互式应用程序页面。 它们包含来自数据库的数据和适应所连接用户的权限的内容。 例如，您可以在外联网中创建编辑表单，或创建通知表单，其中包含来自数据库的数据，以及表、图表、输入表单等。 此功能允许您设计和发布用户可以在其中查找或输入信息的网页。
...
工作流是活动执行流的可视表示形式。 它允许您在应用程序服务器的不同模块之间编排所有流程和任务。 您可以利用这个全面的图形环境设计各式流程，包括部分划分、活动执行、文件处理、人员参与等。工作流引擎会执行并跟踪这些流程。
...
工作流日志是工作流的分步执行日志。 它包含工作流的所有历史记录或审核跟踪。 它用于开发、故障诊断或调试。

详细了解 [工作流日志](../../workflow/using/monitoring-workflow-execution.md).
+++

+++**工作台**

工作表包含工作流过渡所携带的所有信息。 每个工作流均使用多个工作表。工作台保存其原始活动的结果，其内容用作工作流中下一个（已连接）活动的输入。  操作（扩展、自定义）工作表是Adobe Campaign操作员的主要技能之一。

详细了解 [工作台](../../workflow/using/about-workflows.md).
+++
