...
Adobe Campaign允许您定义和管理分配给各个运营商的权限。 这些权限和限制是授权或拒绝的：
...
它们由关联到运营商与Adobe Campaign的连接模式的安全参数完成。 有关详细信息，请参见[此页面](../../installation/using/configuring-campaign-server.md#defining-security-zones)。

有两种类型的权限可授予用户：

* 您可以定义属性权限的运算符组，然后将这些运算符与一个或多个组关联。 这使您能够重复使用权限并使操作员配置文件更加一致。 它还方便了档案的管理和维护。 组创建和管理显示在操作员 [组中](#operator-groups)。
* 您可以将指定权限直接归因给用户，在某些情况下，会使通过组分配的权限过载。 这些权限在指定权 [限中显示](#named-rights)。
...
运营商是具有登录和执行操作权限的Adobe Campaign用户。
...
>[!CAUTION]
>
>操作员需要链接到安全区域才能登录到实例。 有关Adobe Campaign中安全区域的详细信息，请参 [阅本页](../../installation/using/configuring-campaign-server.md#defining-security-zones)。
...
1. 在部分中选择授予操作员的权 **[!UICONTROL Operator access rights]** 限。
   ...
   要向操作员分配权限，请单击位 **[!UICONTROL Add]** 于权限列表上方的按钮，然后从可用组列表中选择一组操作员：

   ![](assets/s_ncs_user_permissions_operators.png)

   您还可以选择一个或多个指定权限(请参阅 [指定权限](#named-rights))。 要执行此操作，请单击字段右侧的箭头， **[!UICONTROL Folder]** 然后选择 **[!UICONTROL Named rights]**:

   ![](assets/s_ncs_user_rights_operators.png)

   选择要分配的组和／或指定权限，然后单击以 **[!UICONTROL OK]** 进行验证。

1. 单击 **[!UICONTROL Ok]** 以创建运算符：配置文件将添加到现有操作符列表。
...
创建操作员的配置文件后，您可以添加或更新其信息。 要执行此操作，请单击选 **[!UICONTROL Edit]** 项卡。
...
>[!NOTE]
>
>不同时区内的协作需要以UTC格式存储日期。 日期在以下上下文中在适当的时区内转换：在用户时区中显示日期时，在导入和导出文件时，在计划电子邮件发送时，在工作流中安排活动时（调度程序、等待、时间约束等）
>
>链接到这些上下文的约束和建议会在Adobe Campaign文档的相关部分中显示。

此外，您还 **[!UICONTROL Regional settings]** 可以通过下拉列表选择显示日期和数字的格式。

### 访问权限选项 {#access-rights-options}

使用选 **[!UICONTROL Access rights]** 项卡可更新链接到操作员的组和指定权限。

![](assets/operator_profile_security_options.png)

The **[!UICONTROL Edit the access parameters...]** link lets you access the following options:

* 通过 **[!UICONTROL Disable account]** 此选项可禁用操作员的帐户：他将不再访问Adobe Campaign。
* 通过 **[!UICONTROL Forbid access from the rich client]** 此选项，可将Adobe Campaign的使用限制为通过 [Web访问](../../platform/using/adobe-campaign-workspace.md#console-and-web-access) 或通过API进行：不再提供对Adobe Campaign客户端控制台的访问权。
...
* 您还可以使用相应的链接定义可信的IP掩码。
   ...
   如果IP地址在此列表中，运营商将能够连接到Adobe Campaign，而无需输入口令。
...
该选 **[!UICONTROL Audit]** 项卡允许您查看与操作员相关的信息。 根据操作员的干预区域中定义的设置自动添加各种选项卡。
...
Adobe Campaign使用技术运营商配置了默认配置的配置文件：管理员（“管理员”）、计费（“计费”）、监控、Web应用程序代理(“webapp”)等 其中一些取决于平台上安装的应用程序和选项：例如，“中心”和“本地”运算符仅在安装了“分布式营销”选项时才可见。

>[!CAUTION]
>
>默认情况下，当平台返回信息消息时，这些技术运营商会收到通知。 我们强烈建议为他们提供联系电子邮件。
>
>为确保Web应用程序正确运行，我们还建议不为“webapp”运算符定义特定的区域设置。
...
1. 从节 **[!UICONTROL Administration > Access management > Named rights]** 点中，单 **[!UICONTROL New]** 击以创建右侧并将其命名为WEBAPP。
   ...
   指定权限在指定权限部分 [中有详细](#named-rights) 说明。

1. 从节 **[!UICONTROL Administration > Access management > Operators]** 点中，选择Web应用程序代理运算符(“webapp”)。
...
1. 单击 **[!UICONTROL New]** 用户组列表右侧的按钮，或右键单击列表并选择 **[!UICONTROL New]**。
...
1. 单击下拉列表或字段右侧的文件夹，以找到要关联 **[!UICONTROL Folder]** 到此组的指定权限或操作符。
...
1. 交付运营商
   ...
   此组中的操作员负责管理交付：它们允许访问创建和准备投放所需的主要资源（营销活动类型、投放映射、默认模板、个性化基块等）。
...
1. 营销活动经理
   ...
   此组中的运营商可以管理营销活动：它允许您访问链接到营销活动的对象（计划、计划、工作流、预算等）。

   此组包含以下指定权限：

   * 插入文件夹：有权将文件夹插入Adobe Campaign树（前提是您对相关分支具有编辑权限）,
   * 工作流：使用工作流。
   >[!NOTE]
   >
   >此组不允许运营商开始交付。
...
1. 内容参与者
   ...
   此组中的操作员可以访问内容管理（可选的Adobe Campaign模块）框 **架内的** “内容”文件夹。 此组不授予任何附加权利。

1. 访问报告

   此组保留给外部操作员，以便通过Web访问访问访问交付报告。

1. 工作流执行

   通过此组，您可以向操作员分配管理与营销活动无关的工作流的权利。

1. 工作流监督者

   如果有关营销活动工作流的警报，此组中的操作员会收到电子邮件通知。

1. 本地／中央管理

   通过这些组，您可以使 **用分布式营销** （可选的Adobe Campaign模块）。

## 指定权限 {#named-rights}

默认情况下，Adobe Campaign会提出一组命名权限，允许您定义分配给操作员和操作员组的授权。 可以从树的节点中编 **[!UICONTROL Administration > Access management > Named rights]** 辑这些权限。

![](assets/s_ncs_admin_named_rights.png)

这些权利如下：

* 管理：通用管理权限应用于控制台上的所有文件夹。
* 批准管理：分配审阅者的权利。
* 中央：集中管理权（分布式营销）。
* 删除文件夹：删除文件夹的权限。
* 编辑文件夹：更改文件夹属性的权利：名称、标签、关联图像等。
* 导出：导出数据的权利。
* 文件访问：有权通过脚本对文件进行读写访问。
* 导入：通用数据导入权限。
* 插入文件夹：插入文件夹的权限。
* 本地：适用于本地管理（分布式营销）的权利。
* 合并：合并记录的权利。
* 准备提交：创建、编辑和开始交付分析的正确方法。
* 隐私数据权：收集和删除隐私数据的权利。 For more on this, refer to this [page](https://helpx.adobe.com/campaign/kb/acc-privacy.html).
* 程序执行：执行外部程序的权利。
* 收件人导入：有权导入收件人。
* SQL SCRIPT执行：在数据库上执行SQL脚本的权限。
* 开始提交：批准先前分析的交货的权利。
* 使用SQL数据管理活动：使用“SQL数据管理”活动编写您自己的SQL脚本，以创建和填充工作表(请参 [阅本节](../../workflow/using/sql-data-management.md))。
* 工作流：使用工作流的权利。
* WEBAPP:使用Web应用程序的权利。
...
默认用户组和指定权限允许操作员访问导航层次结构中的特定文件夹，并授予读取、写入和删除权限。

Adobe Campaign访问权限列表可在此 [处获取](/help/platform/using/assets/accessrights.pdf)。

## 文件夹访问管理 {#folder-access-management}

树的每个文件夹都有其所附加的读取、写入和删除访问权限。 要访问文件，操作符或操作符组必须至少具有对文件的读访问权限。
...
1. 单击选 **[!UICONTROL Security]** 项卡可查看此文件夹的授权。
...
* **替换组或运算符**。 要执行此操作，请单击对文件夹具有权限的组（或运算符）之一，然后从下拉列表中选择一个新组（或新的运算符）:
...
视图是一个文件夹，它显示物理上存储在同一类型的一个或多个其他文件夹中的记录。 例如，如果您创建的营销活动文件夹是一个视图，则默认情况下，该文件夹会显示数据库中存在的所有营销活动，无论其来源如何。 然后，可以过滤此数据。

将文件夹转换为视图时，与数据库中现有的文件夹类型相对应的所有数据都会显示在视图中，而与保存该文件夹的文件夹无关。 然后，您可以对其进行筛选以限制显示的数据列表。

>[!CAUTION]
>
>视图包含数据并提供对它的访问，但数据不实际存储在视图文件夹中。 操作员必须对数据源文件夹中的所需操作（至少读取访问权）拥有相应的权限。
>
>要在不授予对其源文件夹访问权限的情况下授予对视图的访问权限，只是不授予对源文件夹父节点的读取权限。

### 添加文件夹和创建视图 {#adding-folders-and-creating-views}

在以下示例中，我们将创建新文件夹以显示特定数据：

1. 创建新的类 **[!UICONTROL Deliveries]** 型文件夹，并将其命名 **为Deliveries France**。
1. 右键单击此文件夹并选择 **[!UICONTROL Properties...]**。

   ![](assets/s_ncs_user_add_folder_exple.png)

1. 在选项卡 **[!UICONTROL Restriction]** 中，选择 **[!UICONTROL This folder is a view]**。 随后将显示数据库中的所有提交内容。

   ![](assets/s_ncs_user_add_folder_exple01.png)

1. 从窗口中间部分的查询编辑器中定义传送筛选器条件：随后将显示与定义的过滤器对应的营销活动。
