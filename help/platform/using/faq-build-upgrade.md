...
作为托管用户，您无需采取任何操作，即可通过最新稳定版本自动从Campaign年度升级中受益。 内部部署和混合型客户也可以从此版本中受益。 如果您从旧内部版本进行迁移，我们建议您先升级到此版本。[了解详情](../../rn/using/rn-overview.md)。
...
没有。内部版本升级是指给定主版本内的增量更新，而版本升级是指从一个主版本更改为另一个主版本。 构建升级非常简单，因为它们通常不涉及任何重大的体系结构、技术或数据模型更改。
...
没有。在升级期间关闭服务器，以确保在产品升级期间保留数据完整性。 完成后，将重新启动它，并恢复所有服务。
...
没有。在版本升级期间，工作流和邮件服务都会停止。 这意味着不会运行工作流，也不会发送投放。 系统重新启动后，系统将恢复它们。 但是，Adobe强烈建议在升级后检查所有关键路径工作流，以确保它们运行正常。

## 升级期间，我的跟踪链接是否仍然有效？

由于所有服务器都已停止，因此在升级期间，对已发送的电子邮件上的跟踪链接将不起作用。 升级完成后，服务器重新启动后，它们将可以再次运行。

## 在内部版本升级过程中是否需要提供？

是. 客户应在Adobe的生产实例升级期间或升级后立即向客户提供联系人。  Adobe将通过电子邮件联系此人，除非作出其他安排。 这将确保关键任务的顺利过渡和即时验证。 Adobe在内部版本升级完成以进行确认后，将与客户联系。

## 是否需要更新客户端控制台？

是. 客户端控制台必须与服务器实例位于同一内部版本。 升级完成后，客户端控制台应提示您升级到最新内部版本，以确保与服务器内部版本保持一致。
...
是. 在升级期间关闭服务器，以确保在产品升级期间保留数据完整性。 完成后，将重新启动它，并恢复所有服务。
...
* [每年升级计划](../../rn/using/rn-overview.md#yearly-upgrade)
