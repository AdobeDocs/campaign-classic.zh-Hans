...
1. 在相应字段中选择导入类型。有两种可能的导入类型： **[!UICONTROL Simple import]** 以仅导入一个文件，并且 **[!UICONTROL Multiple import]** 以在一次执行中导入多个文件。
...
* 此 **[!UICONTROL Destination schema]** 字段可让您选择数据将导入到的架构。 此信息是强制性的。单击 **[!UICONTROL Select link]** 图标以选择现有架构之一。 单击 **[!UICONTROL Edit link]** 显示选定表的内容。
* 中央表显示源文件中定义的所有字段。选择要导入的字段以将目标文件与它们相关联。可以手动或自动映射这些字段。

  要手动映射字段，请点击复选框以选择源字段，然后点击第二列以激活与所选字段映射的单元格。接下来，单击 **[!UICONTROL Edit expression]** 图标，以显示当前表的所有字段。 选择目标字段并单击 **[!UICONTROL OK]** 以验证映射。
...
* **[!UICONTROL Fixed string]**：对于源文件的所有行，计算字段的值相同。 允许您设置插入或更新的记录的字段值。例如，您可以为所有导入的记录将标记设置为“是”。
...
合并关键项是用于标识记录的一组字段。例如，要导入收件人，协调键值可以是帐号、“电子邮件”字段或“姓氏、名字、公司”字段等。
