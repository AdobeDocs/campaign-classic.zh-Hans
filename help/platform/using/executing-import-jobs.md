...
>[!NOTE]
>
>您可以使用导入数据，而无需将其与数据库数据进行映射。 **[!UICONTROL Import a list]** 函数。 然后，可以通过将数据仅用于工作流中 **[!UICONTROL Read list]** 对象。 有关详细信息，请参见[此页面](../../workflow/using/read-list.md)。
...
1. 单击右侧的箭头 **[!UICONTROL Import template]** 字段以选择您的模板，或单击 **[!UICONTROL Select link]** 以浏览树。
   ...
   本地模板为 **[!UICONTROL New text import]**. 不得修改此模板，但您可以复制此模板，以根据您的要求配置新模板。 默认情况下，导入模板保存在 **[!UICONTROL Profiles and targets > Templates > Job templates]** 节点。

1. 输入此导入的名称 **[!UICONTROL Label]** 字段。 您可以添加描述。
1. 在相应的字段中选择导入类型。 有两种可能的导入类型： **[!UICONTROL Simple import]** 以仅导入一个文件，并且 **[!UICONTROL Multiple import]** 以在一次执行中导入多个文件。
...
1. 通过单击指定要导入的字段 **[!UICONTROL Add]**.
   ...
   每次添加文件时， **[!UICONTROL File to import]** 此时将显示向导。 请参阅部分 [步骤2 — 源文件选择](#step-2---source-file-selection) 并按照向导中的步骤定义用于简单导入的导入选项。
...
* **[!UICONTROL General]** 选项卡
   ...
   * **[!UICONTROL Do not update enumerations]**
     ...
     选择此选项可避免扩充数据库中的枚举值列表。 请参阅 [管理明细列表](../../platform/using/managing-enumerations.md).
...
* **[!UICONTROL Variables]** 选项卡
  ...
  >[!IMPORTANT]
  >
  >此 **[!UICONTROL Variables]** 选项卡仅供工作流类型编程使用，且仅应由专家用户配置。

## 第 2 步 - 源文件选择 {#step-2---source-file-selection}

源文件可以是文本格式（txt、csv、tab、固定列）或 xml。

默认情况下， **[!UICONTROL Upload file on the server]** 已选中。 单击右侧的文件夹 **[!UICONTROL Local file]** 字段以浏览本地磁盘并选择要导入的文件。 您可以取消选择此选项以输入访问路径以及要在服务器上导入的文件名。

![](assets/s_ncs_user_import_wizard02_1.png)

指定文件后，可通过单击在窗口的下半部分查看其数据 **[!UICONTROL Auto-detect format]**. 此预览显示源文件的前 200 行。
...
* **[!UICONTROL Download]** 仅当文件已上传到服务器上时可用。
* **[!UICONTROL Auto-detect format]** 用于重新初始化数据源的格式。 利用此选项，可将原始格式重新应用于已通过 **[!UICONTROL Click here to change the file format...]** 选项。
* 此 **[!UICONTROL Advanced parameters]** 链接允许您筛选源数据并访问高级选项。 在此屏幕中，您可以选择仅导入部分文件。 您还可以定义过滤器，例如根据相应行的值仅导入“Prospect”或“Customer”类型的用户。 这些选项仅供专业 JavaScript 用户使用。
...
单击 **[!UICONTROL OK]** 保存格式，然后单击 **[!UICONTROL Next]** 以显示下一步。
...
* 此 **[!UICONTROL Destination schema]** 字段可让您选择数据将导入到的架构。 此信息是必填的。 单击 **[!UICONTROL Select link]** 图标以选择现有架构之一。 单击 **[!UICONTROL Edit link]** 显示选定表的内容。
* 中央表显示源文件中定义的所有字段。 选择要导入的字段，以将目标文件与其关联。 可以手动或自动映射这些字段。

  要手动映射字段，请单击复选框以选择源字段，然后单击第二列以激活与所选字段对应的单元格。 接下来，单击 **[!UICONTROL Edit expression]** 图标，以显示当前表的所有字段。 选择目标字段并单击 **[!UICONTROL OK]** 以验证映射。

  要自动关联源字段和目标字段，请单击 **[!UICONTROL Guess the destination fields]** 图标（字段列表右侧）。 如果需要，可以修改建议的字段。

  >[!IMPORTANT]
  >
  >在继续执行下一步之前，务必验证此操作的结果。

* 您可以对导入的字段应用转换。 要执行此操作，请单击 **[!UICONTROL Transformation]** 与相关字段相关的列，并选择要应用的转换。
...
* **[!UICONTROL Fixed string]**：对于源文件的所有行，计算字段的值相同。 用于设置插入或更新记录的字段的值。 例如，您可以为所有导入的记录将标记设置为“是”。
* **[!UICONTROL String with JavaScript tags]**：计算字段的值是包含JavaScript命令的字符串。
* **[!UICONTROL JavaScript expression]**：计算字段的值是JavaScript函数的求值结果。 返回的值可以是数字、日期等。
* **[!UICONTROL Enumeration]**：字段的值根据源文件中包含的值而属性。 编辑器允许您指定源列并输入枚举值清单，如以下示例所示：

  ![](assets/s_ncs_user_import_wizard03_3.png)

  此 **[!UICONTROL Preview]** 选项卡允许您查看定义的配置的结果。 在此， **[!UICONTROL Subscription]** 已添加列。 该值通过 **Status** 字段计算。
...
每个节点（表或字段）都有特殊选项可用。 当您单击列表中相关的节点时，其参数和简要说明将显示如下。 为每个元素定义的行为将显示在相应的 **[!UICONTROL Behavior]** 列。
...
* 在 **[!UICONTROL Reject entity]** 模式，则不会导入元素，并在导入日志中生成错误。
* 在 **[!UICONTROL Ignore]** 模式，不会导入元素，但不会保留任何错误痕迹。 此模式可让您优化性能。
...
欲查看这些拒绝的详细信息，请访问以下网站： **[!UICONTROL Rejects]** 选项卡。

![](assets/s_ncs_user_import_rejets_tab.png)

拒绝有两种类型(类型显示在 **[!UICONTROL Connector]** 列)：
...
* **[!UICONTROL Reject parent element]**：拒绝记录的整个行，而不仅仅是导致错误的字段。
...
* 单击 **[!UICONTROL Edit]** 用于选择要与收件人关联或订阅的文件夹、列表或服务的链接。
   ...
   1. 导入到文件夹中
      ...
      >[!NOTE]
      >
      >运算符的默认文件夹是运算符具有写入权限的第一个文件夹。 了解详情，请参阅 [文件夹访问管理](../../platform/using/access-management-folders.md).

      要选择导入文件夹，请单击 **[!UICONTROL Folder]** 字段并选择相关的文件夹。 您也可以使用 **[!UICONTROL Select link]** 图标，以在新窗口中显示树或创建新文件夹。
   ...
   1. 与清单相关联
      ...
      您可以通过单击来为这些收件人创建新列表 **[!UICONTROL Select link]**，则 **[!UICONTROL Create]**. 有关列表创建和管理的信息，请参见 [本节](../../platform/using/creating-and-managing-lists.md).
