...
|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
| **连接到远程数据库** | 仓库使用和数据库权限使用 | 创建链接到AWS帐户的用户 | 创建会话权限 | CONNECT权限 | CONNECT权限 | 创建绑定到具有所有权限的远程主机的用户 |
...
| **创建过程** | 创建模式权限的过程 | 使用ON语言plythonu权限能够调用外部python脚本 | 创建过程或创建任何过程权限 | 创建过程权限 | 使用权限（过程是函数） | 创建例程权限 |
