...
1. 在活动类型规则列表中，单击 **[!UICONTROL New]** 图标。
...
1. 根据需要更改执行顺序。 将多个分类规则应用为 **[!UICONTROL Typology]** 设置，则首先应用顺序较低的规则。 有关更多信息，请参阅 [执行顺序](applying-rules.md#execution-order).
1. 在 **[!UICONTROL Calculation parameters]** 部分，定义频率，以保存下一个每日重新仲裁执行之后的定位。 有关更多信息，请参阅 [调整计算频率](applying-rules.md#adjusting-calculation-frequency).
1. 单击 **[!UICONTROL Pressure]** 选项卡，然后选择应用分类规则的日历期间。
   ...
   >[!NOTE]
   >
   >仅当满足以下条件时，才会考虑已计划的投放： **[!UICONTROL Take the deliveries into account in the provisional calendar]** 选项。 有关更多信息，请参阅 [设置期间](#setting-the-period).
   >
   >此选项在Campaign v8中不可用。
...
1. 指定计算投放权重的方法。
   ...
   每个投放都有一个权重，即表示其优先级别的值：这支持活动之间的仲裁。 权重是使用分类规则和/或其属性中定义的公式计算的。 有关更多信息，请参阅 [消息权重](#message-weight).
...
1. 此 **[!UICONTROL Typologies]** 选项卡允许您查看应用此规则的促销活动分类，或将规则链接到一个或多个现有分类。 有关更多信息，请参阅 [应用分类](about-campaign-typologies.md#applying-typologies).
...
要定义阈值，您可以使用链接到定向维度的维度：例如，要包含投放到访客表中存储的收件人用户档案的消息(有关访客表的更多信息，请参阅 [本节](../../surveys/using/use-case--creating-a-refer-a-friend-form.md))，或避免向在与收件人关联的维度中标识的同一家庭（可能指多个电子邮件地址）每周发送多条消息。
...
请注意，默认情况下，计算阈值时只考虑已发送的投放。 在Campaign Classicv7中，检查 **[!UICONTROL Take the deliveries into account in the provisional calendar]** 选项。 在这种情况下，所考虑的时间段会加倍，以便将未来投放与以往投放整合在一起。
...
1. 输入 **7天** 在 **[!UICONTROL Period considered]** 字段并检查 **[!UICONTROL Take the deliveries into account in the provisional calendar]** 选项：在计算时，将考虑在应用规则的投放日期之前7天内发送的投放以及之后7天内计划的投放。
...
>[!NOTE]
>
>在更改分类规则的定义时，您可以创建 **模拟** 为控制对投放的影响，应将其应用于投放并监控投放彼此产生的影响。 有关更多信息，请参阅 [营销活动模拟](campaign-simulations.md).
...
### 仅发送权重最高的报文 {#sending-only-the-highest-weighted-messages}
...
1. 在 **[!UICONTROL General]** 选项卡，选择 **[!UICONTROL Re-apply the rule at the start of personalization]** 选项。
   ...
   此选项会覆盖 **[!UICONTROL Frequency]** 字段，并在个性化阶段自动应用规则。 有关更多信息，请参阅 [调整计算频率](applying-rules.md#adjusting-calculation-frequency).
...
1. 选择 **[!UICONTROL Take the deliveries into account in the provisional calendar]** 用于包含计划投放的选项。

   ![](assets/campaign_opt_pressure_example_1.png)

   计算时，将考虑在投放日期之前7天内发送的投放以及在投放日期之后7天内计划的投放。 有关更多信息，请参阅 [设置期间](#setting-the-period).

   >[!AVAILABILITY]
   >无法在Campaign v8中考虑计划的投放。
...
1. 将联系日期设置为第二天（当前日期+ 1天）。
   ...
   为了成功实施压力规则排除，请确保在联系日期和时间之前以及在重新应用夜间仲裁之前设置提取日期和时间。 有关更多信息，请参阅 [仲裁后排除](#exclusion-after-arbitration).

1. 取消选择 **[!UICONTROL Confirm the delivery before sending]** 选项并保存更改。
1. 对于要发送的每个投放，以类似方式继续操作。 确保为每个投放设置所需的权重。
1. 运行相关工作流以准备和发送投放。

适用夜间仲裁时，将排除同一收件人权重较低的投放。 只考虑具有最高权重的投放进行发送。 有关更多信息，请参阅 [消息权重](#message-weight).
