...
正在启动 [v7.2.2发布](../../rn/using/latest-release.md#release-7-2-2)，Adobe Campaign依赖于新的可投放性服务器，该服务器可提供高可用性并解决安全性合规性问题。 Campaign Classic现在可将可投放性规则、broadlog和禁止地址从和同步到新的可投放性服务器。 旧的可交付性服务器将于2022年8月31日停用。
...
出于安全合规性的原因，Adobe正在停用旧数据中心。 Adobe Campaign Classic客户端需要迁移到新的可投放性服务，该服务托管在Amazon Web服务(AWS)上。

此新服务器可确保高可用性(99.9)&#x200B;，并提供安全且经过身份验证的端点，以使campaign服务器能够提取所需的数据：新的可投放性服务器不会为每个请求连接到数据库，而是会缓存数据以尽可能为请求提供服务。 这种机制提高了响应时间&#x200B;。

## 您是否受影响？{#acc-deliverability-impacts}

所有客户都受到影响，必须升级到 [Campaign v7.2.2](../../rn/using/latest-release.md#release-7-2-2) 实施其环境以从新的可投放性服务器中获益。

## 如何更新？{#acc-deliverability-update}

As a **托管客户**，Adobe将与您一起将您的实例升级到新版本，并在Adobe Developer控制台中创建项目。

作为 **内部部署/混合部署客户**，您需要升级到 [Campaign v7.2.2](../../rn/using/latest-release.md#release-7-2-2) （或更多）以从新的可投放性服务器中获益。 升级所有实例后，您必须 [实施新的集成](#implementation-steps) Adobe可投放性服务器，并确保无缝过渡。

## 实施步骤 {#implementation-steps}

作为新的可投放性服务器集成的一部分，Campaign需要通过基于Identity Management Service (IMS)的身份验证与AdobeShared Services通信。 首选方式是使用基于Adobe Developer的网关令牌(也称为技术帐户令牌或AdobeIO JWT)。
...
1. 浏览到 **管理>平台>选项**.
1. 检查 `DmRendering_cuid` 选项值已填充。

   * 如果填写了该选项，则可以开始实施。
   * 如果没有填写值，请联系 [Adobe客户关怀](https://helpx.adobe.com/cn/enterprise/admin-guide.html/enterprise/using/support-for-experience-cloud.ug.html){_blank}以获取您的CUID。

   必须在所有Campaign实例(MKT、MID、RT、EXEC)上使用正确的值填充此选项。 作为混合型客户，请联系Adobe以在MID、RT和EXEC实例上设置选项。
...
如果您看不到 **Adobe Campaign** 联系人 [Adobe客户关怀](https://helpx.adobe.com/cn/enterprise/admin-guide.html/enterprise/using/support-for-experience-cloud.ug.html){_blank}以添加它。
...
>[!CAUTION]
>
>作为内部部署客户，如果在您的一侧实施了防火墙，则必须添加此URL `https://deliverability-service.adobe.io` 到你的允许列表。 [了解详情](../../installation/using/url-permissions.md)。
...
1. 选择 **[!UICONTROL Create new project]**。
   ...
   >[!CAUTION]
   >
   >如果您已在将AdobeIO JWT身份验证功能用于其他集成，例如Analytics连接器或Adobe触发器，则必须通过添加来更新项目 **Campaign API** 加入那个项目。
...
1. 如果您的客户端ID为空，请选择 **[!UICONTROL Generate a key pair]** 创建公钥和私钥对。
   ...
   >[!CAUTION]
   >
   >您应该保存 `config.zip` 文件，因为您将无法再次下载，出现下载提示。

1. 单击 **[!UICONTROL Next]**。
1. 选择任何现有 **[!UICONTROL Product profile]** 或根据需要创建一个新版本。 无需权限 **[!UICONTROL Product profile]**. 有关的详细信息 **[!UICONTROL Product Profiles]**，请参阅 [此页面](https://helpx.adobe.com/enterprise/using/manage-developers.html){_blank}。
...
1. 您必须停止，然后重新启动服务器，才能将修改考虑在内。 您还可以运行 `config -reload` 命令。
...
1. 浏览到 **管理>平台>选项**.
1. 访问 `NewDeliverabilityServer_FeatureFlag` 选项并将值设置为 `1`. 应在您的所有Campaign实例(MKT、MID、RT、EXEC)上执行此配置。 作为混合型客户，请联系Adobe以在MID、RT和EXEC实例上设置选项。
...
从2022年7月起，托管客户将开始过渡到新的可投放性服务器(Campaign Managed Services)，从而允许添加这些改进的功能并增强安全性。 所有托管客户都将在8月底之前更新。
...
如果不升级环境，则将停止同步电子邮件设置（ MX管理规则、入站电子邮件规则、域管理规则和退回限定规则）。 这可能会随着时间的推移影响您的可投放性。 如果对这些规则进行了重大更改，则必须从此刻手动应用这些规则。

仅对于MKT实例 [全局禁止显示列表](../../campaign-opt/using/filtering-rules.md#default-deliverability-exclusion-rules) 会受到影响。
