...
下面将找到有关此项目、对营销活动环境的影响以及其他有用资源的一组常见问题。
...
* **数据丢失是否存在风险？**
   ...
   该数据库将从旧版数据中心转储，并在Public Cloud(AWS)中恢复。 在新数据中心重新启动后，应用程序将从关闭前的完全状态恢复。 用户将看不到任何差异，只是某些计划任务将被延迟。

* **旧版数据中心和公共云之间的资源包大小有何差异？**

   我们正在Public Cloud(AWS)中配置基于当前数据库大小、磁盘大小等的新包定义。 例如，如果客户在旧版数据中心中有一个应用程序服务器，则他们可以根据包定义在Public Cloud(AWS)中可以使用两个应用程序服务器。
...
* **如何处理允许列表上的IP?客户是否需要向允许列表中为来自Campaign的传入流量添加新的IP地址？**

   Adobe服务器的IP地址将发生更改。 因此，客户可能需要在其系统的允许列表中添加这些新的IP地址。

   [单击](#config) 此处了解有关允许列表IP的更多详细信息。

* **我们将如何处理添加到允许列表的SFTP/FTP访问端口？**

   SFTP配置(允许列表上的公钥+ IP)也将从旧版数据中心移至公共云(AWS)。 客户没有预期执行的任何操作。

* **我们在改变IP吗？**

   Adobe服务器的IP地址将发生更改。 因此，客户可能需要将这些新的IP地址添加到其系统中的允许列表。
...
* **对于我们用来标记沟通方式的委派子域，应该会产生什么影响？**
   ...
   用于营销通信的子域将保持相同。 但是，根据具体的实施，可能需要在客户端执行以下操作：
   * 如果将子域委派到Adobe（默认），Adobe将处理所有更改并确保无缝过渡。
...
* 出站流量：由Adobe Campaign服务器向您的信息系统或任何其他第三方发起的所有网络活动(例如：短信提供商)。 根据贵组织内的安全策略，更改IP可能需要您的信息系统或任何其他第三方执行允许列表操作
...
* **迁移会影响到到外部帐户的连接吗？**
   ...
   是的。 例如，第三方集成（短信提供商）应当向允许列表添加新的Adobe Campaign应用程序服务器IP地址。

* **迁移是否会影响使用Adobe Analytics连接器与Genesis的连接？要向Adobe Analytics端的允许列表添加促销活动IP地址吗？**

   Adobe Campaign应用程序服务器IP地址将发生更改。 迁移后，Adobe客户关怀将处理此步骤。

* **迁移会影响与其他Adobe解决方案(AEM、Target等)的连接吗？**

   集成是在允许列表和Web服务帐户配置中声明的IP地址组合。 这将由Adobe客户关怀部门负责并拥有。

   随着应用程序服务器IP的更改，外部解决方案中将需要允许列表上的IP地址。 将提供此信息。 集成的其他部分是基于IMS的，应按原样工作。
...
* **迁移是否会影响API连接？**
   ...
   [单击此](#config) 处了解有关IP on 允许列表的更多详细信息。
...
* **如果无法建立SFTP连接，那么为了保持客户的运营，需要采取哪些解决方法/计划？**
   ...
   可能出现的唯一连接问题与客户端的允许列表有关。 客户应在非生产环境中添加此测试，以确保其在移动到生产之前正常工作。

* **是否有任何数据中心特定的允许列表配置需要迁移？**

   不需要，没有要管理的数据中心特定允许列表配置。
...
* **迁移到Public Cloud后，允许多少时间来测试每个实例？**

   根据客户的复杂性，在暂存环境和生产环境迁移之间至少需要1周的烘焙时间。

* **谁将处理向允许列表添加新IP?**

   Adobe客户关怀团队将负责确保客户和任何第三方可以通过向允许列表添加新IP来访问新系统。
