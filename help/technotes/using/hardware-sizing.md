---
product: campaign
title: Campaign Classic v7的硬件大小调整建议
...
>[!CAUTION]
>
>本文仅作为一般示例指南提供。 在开始Campaign项目之前，您必须与Adobe Campaign客户成功经理接洽，以测量部署的确切大小。 **在完成此操作之前，不要**&#x200B;获取或部署任何基础架构或硬件。

本文档提供了在内部部署数据中心或虚拟化云环境中部署Adobe Campaign Classic v7的一般建议。 此类部署（称为&#x200B;**混合**&#x200B;或&#x200B;**中间源**）可将Campaign营销实例和营销数据库置于您的操作控制之下，同时使用Adobe云消息服务发送电子邮件、短信或SMPP消息，并收集电子邮件打开、退回和点击跟踪数据。

营销实例是Adobe Campaign架构的一部分，它驱动着所有营销活动，并存储着营销活动返回的所有收件人数据和分析数据。 营销实例是一组运行Adobe Campaign服务的内部部署服务器和关系数据库。

>[!CAUTION]
>
>如果您使用完全托管的Adobe Campaign实例（在Adobe Cloud Services中部署），本文档中的信息不适用。

[兼容性矩阵](../../rn/using/compatibility-matrix.md)中详细说明了软件兼容性。
...
1. [中等大小](#scenario-1) — 系统中有500万个活动收件人
1. [大型](#scenario-2) — 系统中有2000万活动收件人
...
营销实例中的应用程序服务器需要足够的CPU和内存来运行工作流和响应SOAP API调用，包括Campaign Console用户的请求。 对于使用具有复杂选件规则的出站交互、执行自定义Javascript的工作流以及具有高流量级别的Web应用程序的工作流，CPU要求可能非常重要。
...
为了安全性和可用性，Adobe建议将Internet流量与业务用户生成的流量分开。 因此，图中包含两组服务器：Web服务器（面向Internet的Web1和Web2）和应用程序服务器（业务流程App1和App2）。
...
Adobe为您提供了可在部署环境中中继到Adobe Campaign服务器的URL路径的排他列表。

### 架构

无论卷如何，一般体系结构几乎完全相同。 安全性和高可用性要求要求至少有四台服务器；如果没有使用WebApps，则有两台服务器。 配置的差异主要取决于硬件配置，如CPU核心和内存。
...
对于这些卷，一对Adobe Campaign应用程序服务器系统为Adobe Campaign客户端用户和工作流执行提供了所有功能。 对于500万活跃收件人和此电子邮件卷，应用程序服务器工作负载不需要CPU或I/O密集处理；大部分压力都集中在数据库上。
...
有关代理、首选项中心/订阅处理和磁盘空间使用的更多注释，请参阅[方案1：中等大小部署](#scenario-1)。

### 数据库

数据库服务器的硬件建议如下：

**3Ghz+ 8核CPU，64 GB RAM，RAID 1或10,2个320 GB固态硬盘或RAID 10,640 GB固态硬盘**
...
支持5000万收件人的部署与[方案2](#scenario-2)中所示的部署基本相同：促销活动Web应用程序流量被路由到Campaign Web服务器，因此启动大型促销活动后出现的Web流量激增不会影响促销活动工作流和客户端控制台用户。
...
**3Ghz+ 8核CPU，96 GB RAM，RAID 1或10，至少1.5TB固态硬盘**
...
* **电子邮件促销活动大小**
活动启动频率会影响数据库服务器CPU要求。 与直接邮寄、入站交互和其他工作流相结合，电子邮件营销活动的分段操作给数据库服务器带来了沉重的负载。

* **直邮频率**
直接邮寄的频率可能会影响数据库服务器CPU的要求。 与促销活动启动和其他工作流相结合，直接邮件的分段操作会给数据库服务器带来大量负载。

* **短信消息量**
与电子邮件促销活动大小一样，SMS消息量不会给位于内部部署的Campaign服务器带来大量负载；负载主要位于云端的Adobe Cloud Messaging服务器上。 短信营销活动（如电子邮件和直邮）的分段可能会给营销数据库带来大量负载。 因此，短信促销活动启动频率和分段的复杂性比短信消息量更相关。

* **数据库架构复杂性**
每个活动收件人的数据量既需要存储空间，也需要数据库缓冲区空间，因此通常情况下，更多的收件人需要在数据库服务器上拥有更多的内存和CPU。 复杂的架构还需要连接更多表才能进行分段，因此分段操作可以运行得慢得多，并且在数据跨多个表分布时需要更多数据库CPU和内存。

  数据库服务器内存的估计方法是：确保数据库缓冲池足够大，可以包含所有收件人数据，再加上用于正在运行的工作流的临时表，再加上用于其他数据库操作的利润。

* **出站交互使用情况**
在将所有计算复杂性移交给数据库的工作流中，评估批处理模式下的交互规则。 数据库工作的主要因素是在一个引擎调用期间计算的合格优惠总数（目标大小X每个收件人保留N个最佳优惠前的平均优惠数）。 数据库服务器CPU的速度是性能的首要因素。

* **入站交互或SOAP API使用情况**
入站交互规则和选件会在营销数据库中评估，需要大量数据库服务器资源，特别是CPU。 大量使用入站交互或SOAP API需要单独的Web服务器来将工作负载与运行的活动工作流分开。
...
* **I/O性能**
必须遵循数据库存储的建议的IOPS等级。 Amazon EC2等云服务可能无法提供所需的性能，必须仔细评估。 例如，Amazon EC2预配的SSD卷当前评级为每卷20,000 IOPS。 请参阅[Amazon文档](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ebs-volume-types.html))以了解详情，因此4卷RAID配置的评级为80,000 IOPS，这可能不够。

Adobe建议在系统投入生产之前，对Adobe Campaign的任何虚拟化部署进行性能测试。
