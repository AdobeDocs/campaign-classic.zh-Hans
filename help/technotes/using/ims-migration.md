...
此更改适用于从Campaign Classic v7.3.5（和最新的[IMS迁移兼容版本](ac-ims.md#ims-versions)）开始，并且是&#x200B;**强制性的**，必须将其迁移到Adobe Campaign v8。
...
* Campaign内部部署和混合型客户
  ...
  您可以在[Adobe Developer Console文档](https://developer.adobe.com/developer-console/docs/guides/projects/){target="_blank"}中了解有关Adobe控制台中产品配置文件的更多信息
...
如果此技术帐户用于向消息中心实例进行API调用，请确保为与消息中心关联的营销实例选择Adobe产品配置文件（以messagecenter结尾）。
...
使用Adobe Identity Management System (IMS)身份验证时，要生成WSDL文件，您应在Postman调用中添加授权：持有者&lt;IMS_Technical_Token_Token>：
