...
* **环境**:set包含选件目录和挂接（选件空间）。您需要通过定向维度创建一个环境。 环境有两种类型：
   ...
   * **设计环境**:在环境中创建选件并定义/或分类规则（确定要向目标人员显示或不显示选件的规则）。此外，还定义了将由选件定向的个人表和用于存储所有选件建议的表。 **[!UICONTROL Design environment]**&#x200B;节点包含选件空间子文件夹、预定义过滤器和选件类别。 对于每个&#x200B;**[!UICONTROL Design environment]**，都有一个相应的只读&#x200B;**[!UICONTROL Live environment]**，该只读是从同一&#x200B;**[!UICONTROL Design environment]**&#x200B;生成的。
   * **实时环境**:链接到的环 **[!UICONTROL Design environment]**&#x200B;境。它包含已通过&#x200B;**[!UICONTROL Design environment]**&#x200B;批准其内容和资格的只读选件。 将选择在网站上显示或插入消息。
...
* **统一模式**:每次处理一个联系人。此模式通常用于集客交互和事务型消息。
* **标识模式**:是指联系人的状态。
...
* **权重**:允许您精确计算选件相关性的公式，以便选择最相关的选件。权重在选件中定义。 在减轻重量顺序时，将考虑符合条件的选件。
...
* **选件演示**:渠道用于显示选件的信息。选件表示可以由表示选件的空间的渲染功能构建，或直接输入到界面（例如，在HTML块中）中。 选件可以由空格表示。
