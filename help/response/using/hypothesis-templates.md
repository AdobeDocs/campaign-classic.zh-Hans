...
通过配置假设验证模板，可定义用于衡量反应的上下文，无论对于投放还是选件。 这是引用各种测量表的地方，包括用于定义个人、假设和交易表之间关系的测量表。
...
1. 单击 **[!UICONTROL New]** 或右键单击模板列表，然后选择 **[!UICONTROL New]** 下拉列表中。
1. 输入假设验证标签。
1. 指定模板是要用于优惠上的假设还是要通过 **[!UICONTROL Hypothesis type]**.
1. 对象 **[!UICONTROL Delivery]** 类型模板，指定是否应在控制组的情况下执行测量。 [了解详情](#properties-of-a-hypothesis-template)
1. 对象 **[!UICONTROL Delivery]** 键入模板，您可以选择特定渠道，或决定使用将模板应用于Adobe Campaign中的所有可用渠道 **[!UICONTROL Channel]** 下拉列表。 [了解详情](#properties-of-a-hypothesis-template)
1. 选择 **[!UICONTROL Execution folder]** 其中，您要创建并自动执行将由此模板创建的假设验证。
1. 选择执行设置。 [了解详情](#hypothesis-template-execution-settings)
1. 指定假设验证计算周期。 [了解详情](#hypothesis-template-execution-settings)
...
1. 如果模板配置为 **[!UICONTROL Offer]** 键入假设验证，您可以启用 **[!UICONTROL Update offer proposition status]** 选项：在这种情况下，选择要更改的优惠建议状态。
...
* **[!UICONTROL Hypothesis type]**：用于确定模板是否应该用于投放或选件的假设验证。

   您还可以选择创建一个假设验证，该假设验证将同时应用于投放和选件。

   >[!NOTE]
   >
   >如果模板适用于选件，则 **[!UICONTROL Update offer proposition status]** 选项位于 **[!UICONTROL Transactions]** 选项卡。

* **[!UICONTROL Measurement with control group]**：用于声明是否已为投放或营销活动定义控制组，并将其包含在衡量指标中。 控制组不接收投放，通过将其与接收投放的目标群体进行比较，可衡量投放后营销活动的影响。
...
* **[!UICONTROL Channel]**：您可以选择特定渠道，或通过选择使假设验证模板可用于Adobe Campaign控制台中的所有渠道 **[!UICONTROL All channels]** 下拉列表中。 如果您为特定渠道配置模板，则可在创建假设验证时，自动筛选每个渠道的投放。 [了解详情](creating-hypotheses.md)

   ![](assets/response_properties_001.png)

* **[!UICONTROL Execution folder]**：用于为假设验证指定执行文件夹。
* **[!UICONTROL Taken into account in campaign ROI calculation]**：在计算相关营销活动的ROI时，会将假设验证结果考虑在内。

### 假设验证模板执行设置 {#hypothesis-template-execution-settings}

模板的 **[!UICONTROL General]** 选项卡还允许您指定假设验证执行参数。 可用选项如下：

* **[!UICONTROL Schedule execution for a time of low activity]**：可让您计划假设验证启动以优化Adobe Campaign性能。 选中此选项后，营销活动的处理工作流会在停机期间执行假设验证计算。
...
* **[!UICONTROL Automatic execution]**：如有必要，可让您计划重新计算假设验证（例如，如果您要定期更新指标直到投放结束）。
...
* **[!UICONTROL Log SQL queries in journal]**：此函数为专家用户保留。 它允许您向衡量假设验证审核添加选项卡以显示SQL查询。 这样可以在模拟完成时出现错误时检测可能发生的故障。
* **[!UICONTROL Keep execution workflow]**：用于保留假设验证计算开始时自动生成的工作流。 在从选中了此选项的模板创建的假设中，生成的工作流可用于跟踪该过程。
...
此选项卡包含各种字段和表，可让您保存收件人反应在交易方面的历史记录。 请参阅此 [部分](../../configuration/using/about-schema-reference.md) 了解响应管理专用表格的详细信息。

* **[!UICONTROL Schema (reaction log storage)]**：选择收件人反应表。 Adobe Campaign中的现成表格是 **NmsRemaMatchRcp**.
* **[!UICONTROL Transaction schema]**：选择假设将关注的表，即交易表或购买表。
* **[!UICONTROL Querying schema]**：选择用于筛选假设验证的标准。
* **[!UICONTROL Link to individuals]**：选择个人与用作交易模式的表之间的链接。
* **[!UICONTROL Link to the household]**：如果您希望在假设验证中包含某个家庭的所有成员，请选择交易模式中指向该家庭的链接。 此字段是可选的。
* **[!UICONTROL Transaction date]**：此字段是可选的，但建议使用此字段，因为它允许您定义假设验证计算的范围。
* **[!UICONTROL Measurement period]**：用于配置执行假设和恢复购买行的开始和结束日期。

   当假设与投放关联时，会在直邮投放的联系日期后几天，或电子邮件或短信投放的投放日期后自动触发测量。

   ![](assets/response_measurement_001.png)

   如果这个假设被迅速抛出，那么如果想立即触发它，它可能会被迫抛出。 否则，它会根据配置的计算结束日期（基于假设创建日期）自动触发。 [了解详情](creating-hypotheses.md#creating-a-hypothesis-on-the-fly-on-a-delivery)).

* **[!UICONTROL Transaction/Margin amount]**：这些字段是可选的，可用于自动计算人员调整指标。 [了解详情](hypothesis-tracking.md#indicators)
* **[!UICONTROL Unit amount]**：用于设置用于计算收入的金额。 [了解详情](hypothesis-tracking.md#indicators)
...
定义事务处理表和假设验证将涉及到的字段后，可通过使用过滤器指定目标事务处理和投放来细化假设的范围。 您还可以使用JavaScript脚本明确指向事务表中引用的产品。
...
* **筛选收件人**：在 **[!UICONTROL Scope]** 选项卡，您可以将假设验证限制为与消息关联的任何信息（投放、收件人、电子邮件地址、服务等）：
...
* **脚本**：您可以使用JavaScript脚本在执行期间动态加载假设验证设置。
...
在本例中，我们将为直邮类型投放创建一个假设验证模板。 事务表(**购买** 在我们的示例中)，这些假设将基于包含链接到文章或产品的购买行。 我们希望配置模型，以便在purchase表中创建关于文章或产品的假设验证。
...
1. 在 **[!UICONTROL Scope]** 选项卡，单击 **[!UICONTROL Edit query]** 创建假设筛选条件。
   ...
   因此，创建的模板使您能够对purchase表中的产品或文章运行假设。
