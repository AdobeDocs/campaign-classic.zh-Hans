...
描述性分析向导可通过&#x200B;**[!UICONTROL Tools > Descriptive analysis]**&#x200B;菜单启动：在这种情况下，分析在默认情况下与收件人有关(**nms:收件人**)。 它适用于Adobe Campaign数据库中的所有数据。

![](assets/reporting_descriptive_wz_launch.png)

要分析标准收件人以外的表(**nms:收件人**)，请单击向导最后一步中的&#x200B;**[!UICONTROL Advanced settings...]**&#x200B;链接，然后选择与设置匹配的表，在这种情况下为&#x200B;**cus:individual**:

![](assets/reporting_descriptive_other_schema.png)

如果要对部分数据生成统计信息，可以定义过滤器：要执行此操作，请单击&#x200B;**[!UICONTROL Advanced settings...]**&#x200B;链接并定义要应用的筛选器，如下所示：

![](assets/reporting_descriptive_wz_filter.png)

该分析将只关注16岁及以上、居住在伦敦的收件人库。
...
通过选择项目并右键单击，打开描述性分析向导。 将仅分析所选数据。

![](assets/reporting_descriptive_from_recipients.png)

* 对于一组&#x200B;**收件人**，选择要分析的收件人，然后右键单击并选择&#x200B;**[!UICONTROL Actions > Explore...]**，如上所示。 如果对收件人的列表应用滤镜，则只分析其内容。

   要选择文件夹或当前筛选器中的所有收件人，请使用CTRL+A快捷键。 这意味着，即使未显示的收件人也将被选中。

   有关描述性收件人分析的示例，请参阅：[定性数据分析](../../reporting/using/use-cases.md#qualitative-data-analysis)。
...
使用&#x200B;**[!UICONTROL Qualitative distribution]**&#x200B;模板可以创建有关所有类型数据(例如公司名称、电子邮件域)的统计信息。

[显示表](#displaying-data-in-the-table)中的数据中详细介绍了通过&#x200B;**[!UICONTROL Qualitative distribution]**&#x200B;模板创建的报表的可用配置选项。 [分析人口](../../reporting/using/use-cases.md#analyzing-a-population)中详细介绍了完整示例。

使用描述性分析向导分析数据时，可用选项取决于所选的设置。 详情如下。

### 数据绑定{#data-binning}

选择要显示的变量时，您可以定义数据绑定，即为所选数据配置分组条件。

![](assets/s_ncs_user_report_wizard_031.png)

>[!NOTE]
>
>当使用聚合计算与计算有关的场时，检查&#x200B;**[!UICONTROL The data is already aggregated]**&#x200B;以提高性能。
...
* **[!UICONTROL None]** :通过此选项，可显示变量的所有可用值，而无需进行绑定。
   ...
   >[!CAUTION]
   >
   >应谨慎使用此选项：它会对报表和计算机性能产生重大影响。

* **[!UICONTROL Auto]** :此选项允许您显示n个最频繁表示的值。这些变量是自动计算的，每个变量表示与素材箱数相比的变量百分比。 对于数值，Adobe Campaign会自动生成n个类，将数据排序到中。
* **[!UICONTROL Manual]** :此选项的操作方式与 **[!UICONTROL Auto]** 选项类似，只是您可以手动设置这些值。为此，请单击值表右侧的&#x200B;**[!UICONTROL Add]**&#x200B;按钮。

   在个性化之前，值可以通过Adobe Campaign自动初始化：为此，请输入要生成的素材箱数并单击&#x200B;**[!UICONTROL Initialize with]**&#x200B;链接，如下所示：
...
* **[!UICONTROL Modulo]** :允许您创建数值组。例如，值为10的模允许您创建一个值间隔，该值间隔会按十变十。
...
>[!NOTE]
>
>可以同时显示图表和表中的变量。 为此，请在&#x200B;**[!UICONTROL Table configuration]**&#x200B;窗口中输入变量。 单击&#x200B;**[!UICONTROL Next]**&#x200B;并在图表配置窗口中选择图表类型。 如果表中定义了子维，则不会在图表中显示子维。

单击&#x200B;**[!UICONTROL Variants]**&#x200B;链接可修改图表属性。
...
* **[!UICONTROL Calculated field]** 创建个性化运算符（仅适用于表）。在&#x200B;**[!UICONTROL User function]**&#x200B;字段中可输入要应用于数据的计算。
   ...
   1. 此统计数据不会显示在表中。 您需要取消选中&#x200B;**[!UICONTROL Advanced]**&#x200B;选项卡的&#x200B;**[!UICONTROL Display in the table]**&#x200B;选项。
...
可以更改结果的布局。 为此，请右键单击相关变量，并从快捷菜单中选择输入。
...
要自行生成描述性分析，请从模板&#x200B;**中选择**&#x200B;新建描述性分析选项（如果默认情况下未设置）。

使用&#x200B;**[!UICONTROL Quantitative distribution]**&#x200B;模板，可生成可测量或计数的数据的统计信息(例如发票额、收件人年龄)。

通过&#x200B;**[!UICONTROL Quantitative distribution]**&#x200B;模板创建的分析报告的配置模式在实现示例[定量分析](../../reporting/using/use-cases.md#quantitative-data-analysis)中详述。
...
默认情况下，Adobe Campaign会优惠一系列要为选定数据计算的统计信息。 您可以根据需要更改此列表、添加到它或删除统计信息。
