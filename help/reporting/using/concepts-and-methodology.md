...
量化允许您通过根据条件对值进行分组来简化数据显示。 根据可用的信息，您可以定义年龄组、将电子邮件域分组在一起、限制为值枚举、明确限制数据在专用行或列中显示和分组所有其他数据等。
...
在以下示例中，以开头的所有电子邮件域 **yahoo** (yahoo.fr、yahoo.com、yahoo.be等)，或使用 **ymail** (ymail.com、ymail.eu等) 将在标签下分组 **雅虎！**，以及包含的地址 **rocketmail.com** 域。

![](assets/s_advuser_cube_class_03b.png)

### 动态管理垃圾桶 {#dynamically-managing-bins}
...
### 自动生成回收站 {#generating-bins-automatically}
...
您可以根据需要创建任意数量的度量：然后选择要显示或隐藏在表中的度量。 有关更多信息，请参阅 [显示度量](#displaying-measures).
...
1. 如有必要，并根据运算符，选择操作涉及的表达式。
   ...
   此 **[!UICONTROL Advanced selection]** 按钮可让您创建复杂的计算公式。 如需详细信息，请参阅[此部分](../../platform/using/about-queries-in-campaign.md)。
