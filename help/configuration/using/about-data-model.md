...
如果相关，您可以将默认收件人表与现成的字段一起使用，如[此部分](#default-recipient-table)中所述。
...
在设计Adobe Campaign收件人模型时，可以使用[现成的收件人表](#default-recipient-table)，或决定创建[自定义表](../../configuration/using/about-custom-recipient-table.md)来存储您的营销用户档案。

事实上，如果您的数据模型不符合以收件人为中心的结构，您可以设置其他表作为Adobe Campaign中的定位维度。 例如，当您需要目标家庭、帐户（如手机）和公司/站点，而不是简单的收件人时，这可能是相关的。

>[!NOTE]
>
>在这种情况下，您需要新建一个[目标映射](../../configuration/using/target-mapping.md)。

使用自定义收件人表时需要的所有原则和步骤详见[本节](../../configuration/using/about-custom-recipient-table.md)。
...
* **活动** 模式-应用程序中所承载数据的物理和逻辑结构用XML描述。它遵循 Adobe Campaign 特有的语法，称为模式。有关Adobe Campaign模式的详细信息，请阅读[此部分](../../configuration/using/about-schema-reference.md)。
