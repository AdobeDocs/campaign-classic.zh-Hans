...
* 字段：元素的名称，前面有根据类型定义的前缀(“i”表示整数，“d”表示多次,“s”表示字符串，“ts”表示日期，等等)
   ...
   字段名称通过&#x200B;**sqlname**&#x200B;属性输入，每个类型分别为&#x200B;**`<attribute>`**&#x200B;和&#x200B;**`<element>`**:
...
默认情况下，任何类型的&#x200B;**`<attribute>`**&#x200B;和&#x200B;**`<element>`**&#x200B;元素都映射到数据模式表的SQL字段。 但是，您可以在XML中引用此字段，而不是SQL，这意味着数据存储在包含所有XML字段值的表的备注字段(“mData”)中。 这些数据的存储是观察模式结构的XML文档。

要在XML中填充字段，必须将&#x200B;**xml**&#x200B;属性（值为“true”）添加到相关元素。
...
* 键可以引用表中的一个或多个字段。
* 当键是要填充的模式中的第一个或者它包含值为“true”的&#x200B;**internal**&#x200B;属性时，它称为“primary”（或“priority”）。
* 每个键定义隐式声明一个唯一索引。 通过添加值为“true”的&#x200B;**noDbIndex**&#x200B;属性，可以阻止在密钥上创建索引。
...
可以在源模式中指定要与&#x200B;**pkSequence**&#x200B;属性一起使用的序列的名称。 如果源模式中未提供此属性，则将使用&#x200B;**XtkNewId**&#x200B;默认序列。 该应用程序对&#x200B;**nms:broadLog**&#x200B;和&#x200B;**nms:trackingLog**&#x200B;模式（分别为&#x200B;**NmsBroadLogId**&#x200B;和&#x200B;**NmsTrackingLogId**）使用专用序列，因为这些表包含最多记录。

从ACC 18.10开始，**XtkNewId**&#x200B;不再是现成模式中序列的默认值。 您现在能够构建模式或使用专用序列扩展现有模式。
...
>[!NOTE]
>
>Adobe Campaign模式（例如&#x200B;**NmsTrackingLogId**）中引用的序列必须与SQL函数关联，该函数返回参数中的ID数，以逗号分隔。 此函数必须称为&#x200B;**GetNew** XXX **Ids**，其中&#x200B;**XXX**&#x200B;是序列的名称（例如&#x200B;**GetNewNmsTrackingLogIds**）。 视图随&#x200B;**datakit/nms/eng/sql/**&#x200B;目录中的应用程序提供的&#x200B;**postgres-nmssql**、**mssql-nmssql**&#x200B;或&#x200B;**oracle-nmssql**&#x200B;文件，以恢复“NmsTrackingLogId”的示例为每个数据库引擎创建序列。
