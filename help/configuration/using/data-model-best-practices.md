...
description: 了解在设计收件人模型时如何在Adobe Campaign经典中使用现成的数据表。
...
>[!NOTE]
>
>Adobe Campaign经典允许构建自定义客户表。 但是，在大多数情况下，建议使用已预 [建其他表](../../configuration/using/about-data-model.md#default-recipient-table) 和功能的标准收件人表。
...
>[!NOTE]
>
>Adobe Campaign既不是数据仓库也不是报告工具。 因此，请勿尝试将所有可能的客户及其关联信息导入Adobe Campaign，或导入仅用于构建报表的数据。
...
* 字符串字 **段的长** 度应始终用列进行定义。 默认情况下，Adobe Campaign的最大长度为255，但如果您已经知道字段的大小不会超过较短的长度，则Adobe建议将字段保持较短。
...
除了大多数 **表中** 默认定义的自动项外，您还应考虑添加一些逻辑项或业务项（帐户号、客户端号等）。 它稍后可用于导入／对帐或数据包。 有关详细信息，请参 [阅标识符](#identifiers)。
...
此自定义密钥是外部系统供给Adobe Campaign中的实际记录主密钥。
...
Adobe Campaign主键是所有现成表的自动生成的id，对于自定义表可以是相同的。 For more on this, see [this section](#identifiers).
...
默认情况下，自定义序列的值将介于+1,000到+2.1BB之间。 从技术上讲，通过启用负ID，可获得全范围4BB。 应谨慎使用，当从负数到正数时，将丢失一个id: 在生成的SQLAdobe Campaign中，记录0通常被经典查询忽略。

**相关主题：**
* 有关序列自动 **生成功能的更多** ，请参 [阅此文档](https://helpx.adobe.com/campaign/kb/sequence_auto_generation.html)。
* 有关序列耗尽的更多信息，请观 [看此视频](https://helpx.adobe.com/customer-care-office-hours/campaign/sequences-exhaustion-campaign-classic.html)。

## 索引 {#indexes}

索引对性能至关重要。 在模式中声明密钥时，Adobe将自动在该密钥的字段上创建索引。 您还可以为不使用密钥的查询声明更多索引。
...
1. 转到列表数据库中所有收件人的文件夹。 有关此方面的详细信息，请参 [阅管理用户档案](../../platform/using/managing-profiles.md)。
...
Adobe Campaign既不是数据仓库也不是报告工具。 因此，为了确保Adobe Campaign解决方案的良好性能，数据库的增长应保持在可控范围内。 为此，遵循以下一些最佳实践可能会有所帮助。
...
要防止与高行数相关的任何性能问题，请仅在数据库中保留必要的记录。 任何其他记录应导出到第三方Adobe Campaign仓库，并从数据操作数据库中删除。
