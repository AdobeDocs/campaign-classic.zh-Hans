...
* 结构 `<element>`  :定义  `<element>`   或   `<attribute>`    元素。
...
* **聚合（字符串）**:允许您复制 `<element>`  通过其他模式。 此属性将以“namespace:name”的形式接收模式声明。
...
* **dataPolicy（字符串）**:允许您指定对SQL字段中允许的值的批准约束。 此属性的值包括：
...
* **displayAsField（布尔值）**:如果激活此属性，则为“link”类型 `<element>`  将作为字段显示在架构的树视图（“结构”选项卡）中。 这样，就可以将链接显示为本地字段，并在查询期间更改其行为。 在查询的SELECT中找到元素后，将使用链接目标的值。 在查询的WHERE中找到元素后，将使用链接的基础键。
...
* **fullLoad（布尔值）**:此属性在表单的字段选择期间强制显示表中的所有记录。
...
* **length（字符串）**:max 为“字符串”类型SQL字段的值授权的字符数。
...
* **有序（布尔）**:如果激活了属性(ordered=&quot;true&quot;)，则Adobe Campaign会将元素声明序列保留在XML收集元素中。
...
* **revCardinality（字符串）**:此属性定义两个表之间链接的基数。 它用在“链接”类型中 `<element>`.
...
* **revExternalJoin（布尔值）**:激活后，此属性允许您强制外部连接到相对的链接。
* **revIntegrity（字符串）**:此属性定义目标架构上的完整性。 与“@integrity”属性的值相同。 默认情况下，Adobe Campaign会为此属性提供“正常”值。
* **revLabel（字符串）**:标签。
* **revLink（字符串）**:相对链接的名称。 如果值为“_无_“，目标架构中将不会创建相反的链接。
* **revTarget（字符串）**:目标。
...
* **tableSpace（字符串）**:此属性允许您为表指定新的数据存储表空间（在根上有效） `<element>`)。
* **tableSpaceIndex（字符串）**:此属性允许您为表指定新的索引存储表空间（在根上有效） `<element>`)。
* **target(MNTOKEN)**:在表之间创建链接时接收目标架构的名称。 此属性仅对“link”类型元素处于活动状态。
* **模板（字符串）**:此属性定义对 `<element>` 由多个架构共享的元素。 定义会自动复制到当前架构中。
...
* **xmlChildren（布尔值）**:强制存储每个子代( `<element>  or  <attribute>   ) of the   <element>    element in an XML document.   </element>  </attribute> </element>`
