...
_operation (string)、advanced (boolean)、applicableIf (string)、autoIncrement (boolean)、bellsTo (string)、dataPolicy (string)、dbEnum (string)、defOnDuplicate (boolean)、default (string)、desc (string)、edit (string)、enum (string)、expr (string)、feature (string)、featureDate (boolean)、img (string)、inout (string)、label (string)、length (string)、localizable (boolean (name (ntoken (ntoken)、name (ntoken (ntoken)、not)、not (not (not) ref（字符串）、required（布尔）、sql（布尔）、sqlDefault（字符串）、sqlname（字符串）、sqltable（字符串）、target(MNTOKEN)、template（字符串）、translatedDefault（字符串）、translatedExpr（字符串）、type(MNTOKEN)、user（布尔）、userEnum（字符串）、visibleIf（字符串）、xml（布尔）
...
序列中的 `<attribute>` 元素定义于 `<srcschema>` 不影响数据库中的字段创建顺序。 创建序列将按字母顺序排列。
...
* **applicableIf（字符串）**：通过此属性，您可以将字段设为可选字段。 此 `<attribute>` 当满足约束时更新数据库时将考虑元素。 &quot;applicableIf&quot;接收XTK表达式。
* **autoIncrement（布尔值）**：如果激活此选项，则字段会变为计数器。 这使您可以递增一个值（主要是ID）。 （外部使用）
* **beystsTo（字符串）**：获取共享字段的表的名称和命名空间，并填充声明属性的架构。 (仅用于 `<schema>`)。
* **dataPolicy（字符串）**：用于在SQL或XML字段中指定允许值的批准约束。 此属性的值包括：
...
* **desc（字符串）**：用于插入属性的描述。 此描述显示在界面的状态栏中。
...
* **功能（字符串）**：定义一个特性字段：这些字段用于扩展现有表中的数据，但存储在附件表中。 接受的值包括：
   ...
   * 已共享： `Ft_[type_of_key_of_the_schema_containing_the_characteristic]_[type_of_the_characteristic]`
   ...
   有两种类型的特性字段：简单oà¹字段，其中在特性上授权单个值；以及oà¹多选字段，其中特性链接到可能包含多个值的收集要素。
...
* **标签（字符串）**：链接到字段的标签，通常以界面中的用户为目标。 它可让您避免命名约束。
* **length（字符串）**：最大值 “字符串”类型SQL字段的值的字符数。 如果未指定“@length”属性，Adobe Campaign会自动创建一个包含255个字符的字段。
* **localizable（布尔值）**：如果激活，此属性会告知收集工具恢复“@label”属性的值以进行翻译（内部使用）。
* **名称(MNTOKEN)**：与表中字段的名称匹配的属性的名称。 “@name”属性的值必须较短（最好为英文），并符合XML命名约束。
...
* **pkgStatus（字符串）**：在导出资源包期间，将根据“@pkgStatus”的值考虑值：

   * &quot;always&quot;：始终存在
   * “从不”：从不出现
   * &quot;default(or nothing)&quot;：导出值，除非它是默认值，或者不是与其他实例不兼容的内部字段。

* **ref（字符串）**：此属性定义对 `<attribute>` 由多个架构共享的元素（定义分解）。 该定义不会复制到当前架构中。
* **必需（布尔值）**：如果激活此属性(@required=&quot;true&quot;)，则界面中会高亮显示该字段。 字段的标签在表单中将变为红色。
* **sql（布尔值）**：如果激活此属性(@sql=&quot;true&quot;)，则强制存储SQL属性，即使包含该属性的元素具有xml=&quot;true&quot;属性也是如此。
* **sqlDefault（字符串）**：利用此属性，可定义在激活@notNull属性时用于更新数据库的默认值。 如果在属性创建后添加此属性，则即使对于新记录，架构行为也不会更改。 要更改架构并更新新记录的值，您需要删除并再次创建属性。
* **sqlname（字符串）**：创建表期间字段的。 如果未指定@sqlname，则默认使用“@name”属性的值。 在数据库中写入架构时，将根据字段的类型自动添加前缀。
...
* **translatedExpr（字符串）**：如果存在“@expr”属性，“@translatedExpr”属性允许您重新定义要与@expr中定义的表达式匹配的表达式，该表达式将由翻译工具收集（内部使用）。
* **类型(MNTOKEN)**：字段类型。
...
* **xml（布尔值）**：如果激活此选项，则字段的值没有链接的SQL字段。 Adobe Campaign为记录存储创建文本类型“mData”字段。 这意味着没有对这些字段进行筛选或排序。
