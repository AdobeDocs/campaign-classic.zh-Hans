...
* **appliableIf（字符串）**:此属性允许您将字段设为可选字段。在遵守约束时更新数据库时，将考虑`<attribute>`元素。 “appliableIf”接收XTK表达式。
* **autoIncrement（布尔）**:如果激活此选项，则字段将变为计数器。这样，您就可以递增一个值（主要是ID）。 （外部使用）
* **属于（字符串）**:采用共享该字段的表的名称和命名空间，并填充声明属性的架构。（仅在`<schema>`中使用）。
* **dataPolicy(string)**:允许您指定对SQL或XML字段中允许的值的批准约束。此属性的值包括：
...
* **ref（字符串）**:此属性定义对由多个架 `<attribute>` 构共享的元素的引用（定义保理）。定义不会复制到当前架构中。
