---
title: CRM连接器
description: 进一步了解CRM Connector，并配置与Adobe Campaign的数据同步。
...
1. 转到部 **[!UICONTROL Remote fields]** 分并输入要导入的字段。
   ...
   如有必要，请通过列的下拉列表更改数据格 **[!UICONTROL Conversion]** 式。 本页详细介绍了可能的转 [化类型](../../platform/using/crm-connectors.md#data-format)。
...
1. 您还可以根据需要筛选要导入的数据。 要执行此操作，请单击链 **[!UICONTROL Edit the filter...]** 接。
   ...
   与数据过滤模式链接的限制在“数据过滤 [”部分中有详细说明](#filter-on-data) 。
...
* 比较可能只涉及null值(&#39;is empty&#39;/&#39;is not empty&#39; type)或数字。 这意味着，一旦 **[!UICONTROL Value]** 评估了列（右侧列），此评估的结果必须是数字。
* 列中的数据 **[!UICONTROL Value]** 将用JavaScript进行评估。
* 不支持JOIN比较。
...
1. 在部 **[!UICONTROL Mapping]** 分中，指定要导出的字段及其在CRM中的映射。
   ...
   如有必要，请通过列的下拉列表更改数据格 **[!UICONTROL Conversion]** 式。 本节详细介绍了可能的转 [换类型](../../platform/using/crm-connectors.md#data-format)。
...
出站转 **[!UICONTROL Reject]** 换允许您访问包含与错误消息和代码相关的特定列的输出架构。 这些列包括：
...
通过 **[!UICONTROL Behavior]** 该选项卡可以启用拒绝的处理。 此选项为活动生成第二个输出过 **[!UICONTROL CRM connector]** 渡。 For more on this, refer to this [section](../../platform/using/crm-connectors.md#error-processing).
