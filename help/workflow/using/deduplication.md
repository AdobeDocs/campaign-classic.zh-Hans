...
重复数据删除会从集客活动的结果中删除重复项。 可对电子邮件地址、电话号码或其他字段执行重复数据删除。

此 **[!UICONTROL Deduplication]** activity用于从数据集中删除重复行。 例如，下面的记录可能被视为重复，因为它们具有相同的电子邮件地址和相同的手机和/或家庭电话。

| 上次修改日期 | 名字 | 姓氏 | 电子邮件 | 手机 | 电话 |
-----|------------|-----------|-------|--------------|------
| 02/03/2020 | 鲍勃 | Tisner | bob@mycompany.com | 444-444-4444 | 888-888-8888 |
| 05/19/2020 | 罗伯特 | Tisner | bob@mycompany.com | 444-444-4444 | 777-777-7777 |
| 07/22/2020 | 鲍比 | Tisner | bob@mycompany.com | 444-444-4444 | 777-777-7777 |

此 **[!UICONTROL Deduplication]** activity能够在识别出重复项后，将整行作为唯一记录保留。 例如，在上述用例中，如果活动配置为仅保留具有最旧记录的记录 **[!UICONTROL Date]**，结果将为：
...
1. 选择此活动的目标类型（默认情况下，重复数据删除会链接到收件人）以及要使用的条件，即允许您识别重复项的相同值的字段。
   ...
   >[!NOTE]
   >
   >如果使用外部数据作为输入（例如来自外部文件），请确保选择 **[!UICONTROL Temporary schema]** 选项。
   >
   >在下一步中， **[!UICONTROL Other]** 选项允许您选择要使用的标准或标准：

   ![](assets/s_user_segmentation_dedup_param2.png)

1. 在下一步中， **[!UICONTROL Other]** 选项允许您选择在值相同的情况下使用的标准或标准。
...
1. 从下拉列表中选择要使用的重复数据删除方法，然后输入要保留的重复项数。
   ...
   * **[!UICONTROL Using an expression]**：用于保留具有给定表达式中最低（或最高）值的记录。
   ...
   >[!NOTE]
   >
   >此 **[!UICONTROL Merge]** 功能，可通过 **[!UICONTROL Advanced parameters]** 链接，可配置一组规则，以将一个字段或一组字段合并到单个结果数据记录中。 有关此内容的更多信息，请参阅 [将字段合并到单个记录中](#merging-fields-into-single-record).

1. 单击 **[!UICONTROL Finish]** 以批准选定的重复数据删除方法。

   窗口的中间部分总结了定义的配置。

   在活动编辑器窗口的下部，您可以修改图形对象的叫客过渡的标签，并输入与活动结果关联的区段代码。 此代码以后可用作定位条件。

   ![](assets/s_user_segmentation_dedup_param8.png)

1. 查看 **[!UICONTROL Generate complement]** 选项。 补充包含所有重复项。 然后，将向活动添加其他过渡，如下所示：
...
已识别的重复项还将集成到专用的重复项列表中，如有必要，可重复使用。

![](assets/deduplication_example.png)

1. 添加并链接工作流运行所需的各种活动，如上所示。

   此处使用合并活动将三个查询“统一”为一个过渡。 因此，重复数据删除不适用于单个查询，而适用于整个查询。 有关此主题的更多信息，请参阅 [最佳实践](#best-practices).
...
运行工作流时，将从结果（以及投放）中排除所有标识为重复项的收件人，并将其添加到重复项列表中。 可以再次使用此列表，而不必重新标识重复项。

## 将字段合并到单个数据记录中 {#merging-fields-into-single-record}

此 **[!UICONTROL Merge]** 利用功能，可为重复数据删除配置一组规则，以定义要合并到单个结果数据记录中的字段或字段组。

例如，如果有一组重复记录，您可以选择保留最早的电话号码或最近的名称。
...
1. 选择 **[!UICONTROL Merge records]** 用于激活功能的选项。

   如果要对每个合并条件中的多个数据字段进行分组，请激活 **[!UICONTROL Use several record merging criteria]** 选项。

   ![](assets/dedup2.png)

1. 激活功能后， **[!UICONTROL Merge]** 选项卡将添加到 **[!UICONTROL Deduplication]** 活动。 它允许您定义要合并的字段组及其相关规则。
...
这组三个值标识了重复数据删除产生的目标。 **[!UICONTROL tableName]** 是保存目标标识符的表的名称， **[!UICONTROL schema]** 是群体的模式（通常为nms：recipient）和 **[!UICONTROL recCount]** 是表中的元素数。
