...
product: campaign
title: 技术工作流
description: 进一步了解技术工作流包提供的Campaign Classic。
audience: workflow
...
本节中详细介绍的工作流与不同的Adobe Campaign内置包一起安装。 这些包和相关技术工作流取决于您的许可协议。 内置软件包详见[本节](../../installation/using/installing-campaign-standard-packages.md)。
...
| **删除已阻止的LINE** 用户(deleteBlockedLineUsersV2) | LINE 渠道 | 此工作流确保LINE V2用户在阻止LINE官方帐户180天后删除数据。 |
| **删除隐私请求数据** (deletePrivacyRequestsData) | 隐私数据保护规定 | 此工作流将删除收件人中存储的Adobe Campaign数据。 |
| **投放指标** (deliveryIndicators) | 中间源平台 | 此工作流更新投放的投放跟踪指示器。 默认情况下，此工作流每小时触发一次。 |
| **讨论论坛流程** （新闻组管理） | 营销资源语(MRM) | 此工作流管理来自论坛的通知的投放。 当接收到批准信号时，将触发 |
| **分布式营销流程** (centralLocalMgt) | 中央／本地营销(分布式营销) | 此工作流将处理与使用分布式营销模块相关的开始。 它启动本地活动的创建并管理与订单和活动包可用性相关的通知。 |
| **事件清除** (webAnalyticsPurgeWebEvents) | Web分析连接器 | 通过此工作流，您可以根据“寿命”字段中配置的期间从事件库字段中删除每个。 |
...
| **完整聚合计算(propositionrcp多维数据集)** (agg_nmspropositionrcp_full) | 优惠引擎（交互） | 此工作流会更新优惠建议多维数据集的完整聚合。 默认情况下，每天早上6点触发。 此聚合捕获以下维：渠道、投放、营销优惠和日期。 然后，优惠建议多维数据集用于根据优惠生成报表。 您可以在[本节](../../reporting/using/about-cubes.md)中进一步了解多维数据集。 |
| **已转换的联系人的标** 识(webAnalyticsFindConverted) | Web分析连接器 | 此工作流为在再营销访客后完成购买的网站活动建立索引。 通过此工作流恢复的数据可以在再营销效率报告中访问（请参阅此页）。 |
| **从Adobe Experience Cloud导入受众** (importSharedAudience) | 与Adobe Experience Cloud集成 | 此工作流允许您将受众/区段从不同的Adobe Experience Cloud解决方案导入Adobe Campaign。 |
| **活动中投放的作业** （交付管理） | 营销活动(活动) | 此工作流会触发已批准的投放和开始对外部投放的服务提供商进行后处理。 它还会发送批准通知和提醒。 |
| **服务提供商上的任务** （供应商管理） | 营销活动(活动) | 批准开始后，此工作流将负责处理提供商（向路由器发送电子邮件并进行后处理）。 |
...
| **处理批次事件** (batchEventsProcessing) | 事务性消息执行（消息中心——执行） | 通过此工作流，您可以先将批次事件放入队列，然后再将其与消息模板关联。 |
...
| **同步Facebook粉丝** （syncFacebook粉丝） | 社交网络（社交营销） | 此工作流将Facebook粉丝导入每天早上7点的Adobe Campaign。 |
| **同步Facebook页面** (syncFacebook) | 社交网络（社交营销） | 此工作流每天早7点将Facebook页面与Adobe Campaign同步。 |
| **同步Twitter页面** (syncTwitter) | 社交网络（社交营销） | 此工作流将Twitter关注者导入每天早上7点的Adobe Campaign。 |
