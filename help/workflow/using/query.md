...
1. 选择定位和筛选维度。 默认情况下，目标是从收件人中选择的。限制过滤器的列表与用于投放定位的筛选器相同。
   ...
   有关详细信息，请参见 [定位和筛选维度](building-a-workflow.md#targeting-and-filtering-dimensions).
...
1. 如果您已选择 **[!UICONTROL Filtering conditions]** 在步骤1中或使用 **[!UICONTROL Filters]** > **[!UICONTROL Advanced filter...]** 选项，之后您将必须手动添加筛选条件。
   ...
   您也可以使用表达式生成器并将其与逻辑选项AND、OR和EXCEPT组合来添加更多标准。 然后，您可以预览 **[!UICONTROL Corresponding SQL query...]** 对于您的标准组合。 有关详细信息，请参阅此 [部分](../../platform/using/defining-filter-conditions.md#building-expressions).
...
1. 如果数据属于查询的过滤维度，则只需在可用字段列表中选择该数据，即可在输出列中显示该数据。
   ...
   * 使用创建的新字段 **[!UICONTROL Add]** 按钮显示在输出列列表的右侧。
     ...
     您还可以添加信息集合，例如合同列表、最近5次投放等。 收藏集与字段一致，这些字段可以为同一用户档案具有多个值（1-N关系）。 有关详细信息，请参见 [编辑其他数据](targeting-data.md#editing-additional-data).
