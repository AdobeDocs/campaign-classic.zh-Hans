...
某些事件变量会被各种活动修改或读取。 这些都是字符串类型的变量。 例如，导出会设置 **[!UICONTROL vars.filename]** 变量中指定刚刚导出文件的全名。 所有这些已读取或修改的变量都记录在中 [关于活动](about-activities.md)，在部分中 **输入参数** 和 **输出参数** 活动的所有成员。

### 用例 {#example}

>[!NOTE]
>
>其他工作流用例在以下位置提供： [本节](about-workflow-use-cases.md).
