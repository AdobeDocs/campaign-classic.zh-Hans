...
1. 选择要移动到主记录的收藏集和要更新的字段。
   ...
   输入在标识一个或多个辅助记录后应用于这些记录的规则。 要实现此目的，您可以使用表达式生成器。 有关更多信息，请参阅此](../../platform/using/defining-filter-conditions.md#building-expressions)章节[。例如，通过指定该值是必须保留的所有不同记录中最近更新的值。
