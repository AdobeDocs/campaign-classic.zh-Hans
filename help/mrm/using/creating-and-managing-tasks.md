...
显示此视图时，将自动进行筛选以便仅显示 **[!UICONTROL operator tasks]**. 您还可以使用窗口上半部分中的字段过滤任务。
...
创建任务后，该任务会添加到活动计划和任务列表。 要编辑任务，请从计划中选择该任务或在任务概述中单击其名称，然后单击 **[!UICONTROL Open]** 链接。
...
也可以添加审阅者(请参阅 [审阅者](#reviewers))和参考文档(请参阅 [引用的文档](#documents-referenced))。
...
可使用以下方式选择它们 **[!UICONTROL Resources]** 图标。 单击 **[!UICONTROL Add]** 并选择相关操作员。
...
您可以通过以下方式查看Adobe Campaign操作员配置文件 **[!UICONTROL Edit link]** 图标。
...
除了参与者外，您还可以定义操作员，一旦任务负责人关闭任务，他们将审核任务。 要执行此操作，请单击 **[!UICONTROL Enable task approval]** 选项中位于左侧下角的 **[!UICONTROL Resources]** 窗口。 这可以是单个操作员、一组操作员或操作员列表。
...
您可以在审核者配置窗口的下半部分定义任务的审批计划。 默认情况下，审阅人自提交日期起有三天时间批准任务。 可以配置提醒，该提醒将在批准截止日期之前自动发送给相关操作员。
...
可以将文档和营销资源添加到任务(有关更多信息，请参阅 [管理营销资源](../../mrm/using/managing-marketing-resources.md))。 为此，请打开任务并单击 **[!UICONTROL Documents]** 图标。
...
>[!NOTE]
>
>有关费用和预算的更多信息，请参阅 [成本承诺、计算及计费](../../mrm/using/controlling-costs.md#cost-commitment--calculation-and-charging).
...
>[!NOTE]
>
>有关管理预算和成本的方法，请参见 [控制成本](../../mrm/using/controlling-costs.md).
