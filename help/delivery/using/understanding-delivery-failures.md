...
* **[!UICONTROL Ignore]** :活动MTA会忽略弹回邮件，这意味着此弹回邮件永远不会导致收件人地址被隔离。 刷新不会用于交付性工 **作流** ，也不会将其发送到客户端实例。
...
当电子邮件失败时，远程服务器会返回一条弹回消息至平台参数中指定的地址。 Adobe Campaign将每个弹回邮件的内容与规则列表中的字符串进行比较，然后将其分配为三个错误类 [型之一](#delivery-failure-types-and-reasons)。

>[!NOTE]
>
>用户可以创建自己的规则。 在导入包时以及通过刷新以实现可交付性工作 **流更新数据时** ，将覆盖用户创建的规则。
...
Adobe Campaign消息服务器将单个域管 **理规则应用** 到所有域。

<!--![](assets/tech_quarant_domain_rules_02.png)-->

* 您可以选择是否激活某些标识标准和加密密钥以检查域名，如 **Sender ID**、 **DomainKeys**、 **DKIM**&#x200B;和 **** S/MIMEConnect。
* 通过 **SMTP relay** 参数，可以为特定域配置中继服务器的IP地址和端口。 For more on this, see [this section](../../installation/using/configuring-campaign-server.md#smtp-relay).

如果您的邮件在Outlook中的发送者地 **[!UICONTROL on behalf of]** 址中显示，请确保您的电子邮件没有使用 **Sender ID**（Microsoft提供的过时的专有电子邮件身份验证标准）进行签名。 如果选 **[!UICONTROL Sender ID]** 项已启用，请取消选中相应的框，然后与Adobe Campaign支持部门联系。 您的可交付性不会受到影响。
