...
请参阅 [各种PDU中的信息](sms-protocol.md#information-pdu) 部分，了解中可用的字段 `BIND PDU`，通常您需要添加短代码 `address_range`，但这需要提供商的特殊支持。 联系他们，了解他们希望如何独立路由多个短代码。
...
* 如果所有内容在中间服务器上都可正常使用，并且短信已正确发送，但营销实例未正确更新，则您可能会遇到中间同步问题。
...
* 如果连接成功但不稳定，请检查 [连接不稳定问题](troubleshooting-sms.md#issues-unstable-connection) 部分。
...
* 连接持续不到1小时。 由于Adobe Campaign Classic MTA的工作方式，Adobe Campaign Classic发射器连接属于例外。
...
* 如果提供程序通过发送 `TCP FIN` 或 `TCP RST packet`，请向您的提供商咨询更多信息。
...
* 检查连接是否稳定。 除Adobe Campaign Classic上的发射器外，SMPP连接应持续保持至少1小时。 请参阅部分 [连接不稳定问题](sms-protocol.md#issues-unstable-connection).

* 如果重新启动MTA使发送MT在短时间内再次正常工作，则可能由于连接不稳定而出现限制。 请参阅部分 [连接不稳定问题](troubleshooting-sms.md#issues-unstable-connection).
...
* 如果您看到许多 `BIND/UNBIND`，则表示您的连接不稳定。 请参阅[连接不稳定问题](troubleshooting-sms.md#issues-unstable-connection) 部分，了解在尝试解决重复消息问题之前的解决方案。
...
如果您修复了所有问题，但某些无效SR仍然位于提供程序的缓冲区中，则可以使用“无效ID确认计数”选项跳过它们。 应小心使用它，并在清除缓冲区后尽快将其重置为0。

## 处理MO（和阻止名单/自动回复）时的问题{#issue-process-MO}
...
* 如果 `DELIVER_SM PDU` 显示，检查Adobe Campaign是否确认成功 `DELIVER_SM_RESP PDU` （代码0）。 此RESP可确保Adobe Campaign阻止列表已应用所有处理逻辑（自动回复和允许/）。 如果不是这种情况，请在短信进程日志中搜索错误消息。
...
* 检查隔离表和投放日志中的电话号码格式是否完全相同。 如果不是，请参考这个 [部分](sms-protocol.md#automatic-reply) 如果您遇到国际电话号码格式的加号前缀问题。
...
网络捕获并不总是需要的，通常冗长的SMPP消息就足够了。 下面是一些准则，可帮助您确定是否需要网络捕获：
