...
* 如果出现`DELIVER_SM PDU`，请检查Adobe Campaign是否确认它并成功`DELIVER_SM_RESP PDU`（代码0）。 此RESP可确保Adobe Campaign已应用所有处理逻辑（自动回复和允许/）。 如果不是这种情况，请在短信进程日志中搜索错误消息。
...
打开有关编码问题的票证时，无论是否向提供程序或[Adobe客户关怀](https://helpx.adobe.com/cn/enterprise/admin-guide.html/enterprise/using/support-for-experience-cloud.ug.html)打开票证，都始终包含您所键入内容和所看到内容的十六进制版本。
...
* **已发送给提供程序**：
消息已使用`SUBMIT_SM command`发送到提供程序，但未从提供程序收到`SUBMIT_SM_RESP`确认。
