...
### 隔离与阻止列表隔离{#quarantine-vs-denylist}
...
在投放准备阶段期间，投放仪表板的投放日志中会列出特定投放的隔离地址（请参阅[投放日志和历史记录](../../delivery/using/delivery-dashboard.md#delivery-logs-and-history)）。
...
### 确定投放报告{#identifying-quarantined-addresses-in-delivery-reports}中的隔离地址
...
### 确定收件人{#identifying-quarantined-addresses-for-a-recipient}的隔离地址
...
Adobe Campaign根据投放失败类型和在错误消息鉴别过程中分配的原因（请参阅[退回邮件鉴别](../../delivery/using/understanding-delivery-failures.md#bounce-mail-qualification)）和[投放失败类型和原因](../../delivery/using/understanding-delivery-failures.md#delivery-failure-types-and-reasons)。
...
* 可修改错误计数阈值。有关更多信息，请参阅[投放临时失败后重试](../../delivery/using/understanding-delivery-failures.md#retries-after-a-delivery-temporary-failure)。
...
使用SMPP协议发送短信消息时，错误管理的处理方式不同。 有关扩展通用SMPP连接器的更多信息，请参阅[此页面](../../delivery/using/sms-set-up.md#creating-an-smpp-external-account)。
...
>[!NOTE]
失败类型和失败原因与电子邮件相同。 请参阅[投放失败类型和原因](../../delivery/using/understanding-delivery-failures.md#delivery-failure-types-and-reasons)。
...
* 错误消息的第二部分（本示例中的&#x200B;**Generic**）引用SMSC实现的名称，如SMS外部帐户的&#x200B;**[!UICONTROL SMSC implementation name]**&#x200B;字段中定义的名称。 请参阅[此页](../../delivery/using/sms-set-up.md#creating-an-smpp-external-account)。
...
* 管道符号(|)之后的所有内容仅显示在&#x200B;**[!UICONTROL Delivery log qualification]**&#x200B;表的&#x200B;**[!UICONTROL First text]**&#x200B;列中。 在消息被标准化后，此内容始终由&#x200B;**#MESSAGE#**&#x200B;替换。 此过程可避免因类似错误而包含多个条目，且与电子邮件相同。 有关更多信息，请参阅[退回邮件鉴别](../../delivery/using/understanding-delivery-failures.md#bounce-mail-qualification)。
