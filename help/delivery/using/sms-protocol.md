...
>[!NOTE]
>
>在此[页面](https://experienceleague.adobe.com/docs/campaign-standard/using/administrating/configuring-sms/sms-protocol.html#administrating)中可找到Adobe Campaign Standard的&#x200B;**SMS连接器协议和设置**。
>
...
此文档将引导您完成Adobe Campaign与SMPP提供商之间建立的连接。
...
* 文本（限制为140字节），表示70到160个字符，具体取决于编码。 有关详细信息和限制，请参见下面的[SMS文本编码](../../delivery/using/sms-protocol.md#sms-text-encoding)。
...
Adobe Campaign Classic支持SMPP协议3.4版。这是一种广泛的协议，允许向提供商(SMSC)发送SMS，并接收SMS和接收。 有关详细信息，请参阅[SMPP文档](https://smpp.org/SMPP_v3_4_Issue1_2.pdf)。
...
在Adobe Campaign Classic，要将SR与其相应的MT链接，SMSC将返回一个ID，其步骤为`SUBMIT_SM_RESP`和`DELIVER_SM`。 标识符存储在`nms::providerMsgId`表的`providerId`字段中，并链接到`broadLogId`和`deliveryId`。 此匹配操作由SMS进程在写入数据库时完成。
...
* **short_message**:文本。对于SR，它包含SMPP协议规范附录B中描述的数据。 有关详细信息，请参见[SR错误管理](../../delivery/using/sms-protocol.md#sr-error-management)。

Adobe Campaign可以读取`receipted_message_id`可选字段中的消息ID，其中包含一些配置。

#### DELIVER_SM_RESP {#deliver-sm-resp}

此PDU由Adobe Campaign发送以确认SR和MO。

Adobe Campaign Classic承认SR和MO一旦被插入数据库。 即使已发送成功的`DELIVER_SM_RESP` PDU，也会发生一些处理错误。 这种限制是由Adobe Campaign Classic的软件架构造成的。
...
多部分SMS或长SMS是多部分发送的SMS。 由于移动网络协议中的技术限制，SMS不能大于140字节，否则需要拆分。 请参阅[SMS文本编码](../../delivery/using/sms-protocol.md#sms-text-encoding)部分，进一步了解适合SMS的字符数。
...
大多数提供商要求每个SMPP连接都有吞吐量限制。 这可以通过在外部帐户中设置多个SMS来实现。 请注意，每个连接发生吞吐量限制，总有效吞吐量是每个连接的限制乘以连接总数。 这在[同时连接](../../delivery/using/sms-protocol.md#connection-settings)一节中有详细介绍。
...
Adobe Campaign Classic连接器（扩展SMPP除外）使用硬编码行为，这取决于所选的提供商。 通用SMPP仅区分成功和错误，没有详细信息。 投放日志可能包含某些不保证的信息。
...
### Adobe Campaign Classic扩展通用SMPP {#sr-processing}中SR处理的示例
...
如果编码问题&#x200B;**，应始终与SMSC提供者联系。**&#x200B;只有SMSC提供商了解他们支持的编码以及可能由于技术平台限制而适用的特殊规则。
...
* 确保您知道哪些字符属于哪个编码。 GSM7不完全支持音符或重音。 尤其是法语，在法语中，é和è是GSM7的一部分，但ê或ï不是。 西班牙语也是如此。
...
设置SMSC实现的名称。 它应设置为提供者的名称。 请与管理员或交付能力团队联系，了解要在此字段中添加的内容。 在[SR错误管理](../../delivery/using/sms-protocol.md#sr-error-management)部分中介绍了此字段的角色。
...
在Adobe Campaign Classic，它定义每个MTA子项的连接数。

Adobe Campaign Classic扩展SMPP连接器可以控制每个MTA子项的连接数。 要控制连接的全局限制，您必须限制MTA子进程的数量，这通常意味着有一个专用的SMS中间源平台。

对于Adobe Campaign Classic来说，可以有不同数量的接收机和发射机连接：

* **发射器连接= MTA子连接数* MTA子进程数* MTA子进程数* MTA数（如果设置了自动回复）+ MTA子连接数**

如上所述，如果启用了自动回复，Adobe Campaign ClassicSMS进程将打开更多的发射器连接。 这些附加连接用于发送自动回复。

* **接收器连接= MTA子连接数**

如果设置自动回复，SMS进程将打开发送器／接收器对，增加发送器连接数。 如果未设置任何自动回复，它将仅打开接收器连接。
...
在Adobe Campaign Classic，日志输出位于与MT相关的流量的MTA日志中，以及与MO和SR相关的流量的SMS日志中。
...
这些设置适用于在发射机+接收机模式下的接收机。 它们的工作方式与发射器部分类似，有关更多详细信息，请参阅上文。
...
有关编码过程的更一般说明，请参见[定义编码设置的特定映射。](../../delivery/using/sms-protocol.md#SMSC-specifics)
...
当网络链路具有高延迟时，该窗口有助于提高吞吐量。  窗口的值必须至少是SMS/s的数量乘以链接的延迟
...
有关文本编码的详细信息，请参见[SMS文本编码](../../delivery/using/sms-protocol.md#sms-text-encoding)部分。
...
选中后，长SMS将发送到一个SUBMIT_SM PDU中，并将文本放在message_payload可选字段中。 有关此的详细信息，请参见[SMPP规范](../../delivery/using/sms-protocol.md#ACS-SMPP-connector)。
...
SR格式并非严格由SMPP协议规范强制执行。 它只是说明书[附录B](../../delivery/using/sms-protocol.md#sr-error-management)（第167页）中所述的建议。 某些SMPP实现程序以不同的格式设置此字段，因此Adobe Campaign需要一种方法来提取正确的字段。
...
默认情况下，以`DELIV`开头的stat值，如[附录B](../../delivery/using/sms-protocol.md#sr-error-management)中的`DELIVRD`将被视为已成功传送，且所有与错误匹配的stat值，如`REJECTED`、`UNDELIV`被视为错误。
...
仅当禁用&#x200B;**消息有效负荷**&#x200B;设置时，此设置才起作用。 如果消息需要的SMS大于此值，则将触发错误。
...
发送投放部件时，MTA生成MTA子对象。 MTA子进程的数量是动态的，取决于serverConf.xml中的配置。 每个MTA子级都实例化连接到SMPP提供程序的连接器`CSmppConnectorWorker`。 只要MTA子项保持活动状态，连接就保持活动状态，也可在serverConf.xml中进行配置。
...
当MT和SR都完成处理后，您应该有完整的行，其中同时包含广播信息和状态信息。

此处，`iMsgId`链接到`nmsBroadLogMsg`表，表示完整状态／错误消息。
...
每个MTA子项创建一个可配置的连接数量，因此限制MTA子项的数量将限制连接数量。 由于MTA子进程与流量之间的相关性是相关的，因此这可以有所控制，但仍有些不可预测。
...
检查`BIND_* PDU`s不太多。如果其中太多，则表明连接不稳定。 有关详细信息，请参见[连接不稳定的问题](../../delivery/using/sms-protocol.md#issues-unstable-connection)部分。
