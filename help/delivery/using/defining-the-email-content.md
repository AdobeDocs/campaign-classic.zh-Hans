...
description: 了解在使用Adobe Campaign时如何定义电子邮件内容
...
消息的主题在相应字段中配置。 您可以直接在字段中输入它，或单击 **[!UICONTROL Subject]** 用于输入脚本的链接。 个性化链接允许您在主题中插入数据库字段。
...
例如，在上面的消息中，使用来自收件人用户档案的数据，为每个收件人个性化消息主题。

>[!NOTE]
>
>有关个性化字段的使用，请参见 [关于个性化](about-personalization.md).

您还可以使用将表情符号插入主题行 **[!UICONTROL Insert emoticon]** 弹出窗口。
...
根据收件人首选项，默认情况下将以HTML或文本格式发送消息。 我们建议创建两种格式的内容，以确保在任何邮件系统中均可正确显示消息。 有关更多信息，请参阅 [选择消息格式](email-parameters.md#selecting-message-formats).
...
通过Adobe Campaign，您可以尝试使用新的交互式 [AMP for Email](https://amp.dev/about/email/) 格式，在特定条件下允许发送动态电子邮件。

有关更多信息，请参阅[此章节](defining-interactive-content.md)。

## 使用内容管理 {#using-content-management}

您可以使用内容管理表单，直接在投放向导中定义投放的内容。 要执行此操作，您必须引用要使用的内容管理的发布模板，在 **[!UICONTROL Advanced]** 选项卡中显示的投放属性。

![](assets/s_ncs_content_in_delivery.png)

通过附加选项卡，可输入将根据内容管理规则自动集成和格式化的内容。
...
在投放向导中打开包含图像的HTML内容时，根据投放参数，会显示一条消息，为您提供立即上传图像的选项。
...
* 如果图像存储在本地或位于其他服务器上，即使该服务器从外部可见（例如在Internet网站上），也将被检测为 **[!UICONTROL Not yet online]**.
* 图像被检测为 **[!UICONTROL Already online]** 如果在创建其他投放时提前上传，则为。
* 在部署向导中，您可以定义未启用图像检测的URL：上传这些图像将 **[!UICONTROL Skipped]**.

>[!NOTE]
>
>图像通过其内容而非访问路径进行标识。 这意味着之前以不同名称或不同目录上传的映像将被检测为 **[!UICONTROL Already online]**.

在分析阶段，图像会自动上传到服务器，以便可从外部访问它们，但必须预先上传的本地图像除外。

您可以继续工作并上传图像，以便其他Adobe Campaign操作员能够查看这些图像。 如果相互协作，您会发现此工具非常有用。 要执行此操作，请单击 **[!UICONTROL Upload the images straightaway...]** 将图像上传到服务器。

![](assets/s_ncs_user_images_in_delivery_wiz_3.png)

>[!NOTE]
>
>然后修改电子邮件中图像的URL，尤其是其名称。

图像联机后，您可以从 **[!UICONTROL Source]** 选项卡中显示的消息。
...
条形码生成模块允许您创建符合许多常见标准（包括2D条形码）的多种类型的条形码。

可以使用使用客户标准定义的值动态地生成条形码作为位图。 电子邮件营销活动中可包含个性化的条形码。 收件人可以打印邮件并将其显示给发行公司进行扫描（例如，在结帐时）。

要在电子邮件中插入条形码，请将光标置于要显示条形码的内容中，然后单击个性化按钮。 选择 **[!UICONTROL Include > Barcode...]**。
...
1. 此 **[!UICONTROL Height]** 字段允许您通过更改每条条形码之间的间距来配置条形码的高度，而不更改条形码的宽度。

   根据条形码的类型，没有限制性条目控制。 如果条形码值不正确，它将仅在 **预览** 将条形码以红色划掉的模式。

   >[!NOTE]
   >
   >分配给条形码的值取决于其类型。 例如，EAN-8类型应恰好有8个数字。
   >
   >右侧的个性化按钮 **[!UICONTROL Value]** 字段允许您在值本身之外添加数据。 这丰富了条形码，前提是条形码标准接受它。
   >
   >例如，如果您使用GS1-128类型条形码，并且除了输入值外，还想输入收件人的帐号，请单击个性化按钮并选择 **[!UICONTROL Recipient > Account number]**. 如果正确输入了所选收件人的帐号，则条形码会考虑该帐号。

配置这些元素后，即可完成电子邮件并发送它。 为避免出现错误，请始终确保内容显示正确，然后再通过单击 **[!UICONTROL Preview]** 选项卡。

![](assets/barcode_insert_10.png)

>[!NOTE]
>
>如果条形码的值不正确，则其位图将以红色划掉。
