...
JavaScript模板是包含JavaScript代码的HTML或文本文档。 其构建方式与交付操作中的电子邮件内容相同。

### JavaScript模板的标识 {#identification-of-a-javascript-template}

JavaScript模板的名称和命名空间与其架构和表单一样进行标识。 但是，建议将 **.js** 选项添加到模板名称。
...
* 合并字段：显示数据内容，其语法 **`<%= <source> %>`** 是要 `<source>`显示的数据的源字段。
* 指令块：执行包含在&lt;%和%>标记之间的一系列JavaScript指令。

内 **容对象** ，表示输入XML文档的主要元素。

在我们的示例中，以下行显示名称书名的内容：
...
您可以构成一个函数或变量库，供以后使用。 为此，请导入包含eval函数的JavaScript **模板** 。 这样，您就可以使用其他JavaScript模板中声明的其他函数来丰富上下文。
...
通过编辑区域，您可以填充JavaScript模板的内容：

![](assets/d_ncs_content_form16.png)

>[!NOTE]
>
>必须填充关联的数据模型架构以初始化JavaScript对象。
...
1. 然后，您可以在分发中使用此内容模板。
   ...
   有关此功能的详细信息，请参 [阅使用内容模板](../../delivery/using/using-a-content-template.md)。

## XSL样式表 {#xsl-stylesheets}

XSLT语言允许您将XML文档更改为输出文档。 根据样式表的输出方法，可以在HTML、纯文本或其他XML树中生成生成的文档。

此转换又在称为样式表的文档中以XML格式详细介绍。

### 标识样式表 {#identifying-a-stylesheet}

样式表由其名称和命名空间来标识，就像架构和表单一样。 但是，建议将 **.xsl** extension添加到样式表名称。
...
* 元素必须具有开启标记和结束标记，
* 将“&lt;”或“&amp;”字符替换 **为“&lt;&#39;** 或 **&#39;&amp;&#39;实体** ,
* 每个XSL元素都必须使用 **xsl** namespace。

样式表必须以XSL根元素标记开头， **`<xsl:stylesheet>`** 并以标记结 **`</xsl:stylesheet>`** 尾。 XSL命名空间必须在打开标记中定义，如下所示：
...
默认情况下，XSLT处理器会搜 **索应用于输入** XML文档的根节点或主节点的模板。 输出文档的构建从此模板开 **始**。

在我们的示例中，通过显示书籍名称和章节列表，从“cus:book”架构生成HTML页面。
...
要显示 **html字段** ，请使用指 **令中的disable-output-escenine=** &quot;yes&quot; **`<xsl:value-of>`** 选项。 这样，您就可以避免用字符的XML实体替换字符（例如&lt;和&lt;）。

使用 **`<xsl:text>`** 包含disable-output- **** esceine=&quot;yes&quot;选项的指令可以为个性化字段或条件测试插入JavaScript标记。
...
* 添加条件测 **试&lt;% if(recipient.language == &#39;en&#39;){ %>**:
...
可以构建一个模板库或变量库，以便在多个样式表之间共享。 上面介绍的“ **longMonth”模板**，是一个典型的示例，其优点是在样式表中远程定位模板，以便以后可以重用。
...
相对引用允许您在 **NcmRessourcesDir和NcmRessourcesDirPreview选项中输入包含图像的** 服务器的URL **** 。 这些选项包含要在Adobe Campaign客户端控制台中发布和预览的图像的位置。

这两个选项可通过文件夹中的选项管理屏幕进行 **[!UICONTROL Administration > Platform > Options]** 访问。
...
在样式表处理过程中，输入XML文档的主元素上的 **_resPath** 属性会根据上下文（预览或发布）自动填充一个或多个选项。

如何使用图像放置选项及其与图像的用法示例：
...
您还可以使用声 **[!UICONTROL Public resources]** 明图像并将它们上传到服务器，具体取决于在部署向导中输入的实例设置。
...
>[!NOTE]
>
>有关如何配 **[!UICONTROL Public resources]** 置和使用它们的详细信息，请参阅 [本节](../../installation/using/deploying-an-instance.md#managing-public-resources)。

## 日期显示 {#date-display}

在XML输入文档中，日期以内部XML格式存储： **YYYY/MM/DD HH:MM:SS** （例2018/10/01 12:23:30）。

Adobe Campaign为JavaScript模板和XSL样式表提供日期格式化功能，详见下文。

### JavaScript日期格式 {#javascript-date-formatting}

要以所需格式显示日期，Adobe Campaign提供 **formatDate** 函数，该函数将输入日期的内容，并提供一个字符串，该字符串使用以下语法指定输出格式： **%4Y/%2M/%2D %2H%2N%2S**
...
XSLT语法中没有标准的日期管理功能。 要按所需格式显示日期，Adobe Campaign提供外部函数date- **format**。 此函数将日期的内容作为输入，并使用以下语法指定输出格式的字符串： **%4Y/%2M/%2D %2H%2N%2S**
