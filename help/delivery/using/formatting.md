...
各种JavaScript指令以下列形式显示：

* 合并字段：显示数据的内容 **`<%= <source> %>`** 语法，其中 `<source>`是要显示的数据的源字段。
...
编程语言的所有能力（变量、循环、条件测试、函数等）。 )可用于构造输出文档。 可以访问SOAP API以扩充输出文档。
...
您可以构建一个函数或变量库以供将来使用。 要实现此目的，请使用导入JavaScript模板 **估计** 函数。 这使您可以使用在其他JavaScript模板中声明的其他函数来扩充上下文。
...
XSLT语言允许将XML文档更改为输出文档。 根据样式表的输出方法，生成的文档可以用HTML、纯文本或其他XML树生成。

此转换在称为样式表的文档中的XML中依次是详细的。

### 标识样式表 {#identifying-a-stylesheet}

样式表由其名称和命名空间标识，就像架构和表单一样。 但是，建议您添加 **.xsl** 样式表名称的扩展。

样式表的标识键是由命名空间和用冒号分隔的名称组成的字符串；例如： **cus：book.xsl**.

### 样式表的结构 {#structure-of-a-stylesheet}

基于示例架构“cus：book”的HTML格式化样式表示例：
...
样式表必须以XSL根元素标记开头 **`<xsl:stylesheet>`** 并以 **`</xsl:stylesheet>`** 标记。 必须在开始标记中定义XSL命名空间，如下所示：
...
默认情况下，XSLT处理器会查找 **模板** 应用于输入XML文档的根节点或主节点。 输出文档的构建以如下内容开头 **模板**.

在我们的示例中，通过显示书名和章节列表，从“cus：book”架构生成了一个HTML页面。
...
要显示 **html** 字段，使用 **disable-output-escaping=&quot;yes&quot;** 选项来自 **`<xsl:value-of>`** 指令。 这样，您就可以避免将字符替换为XML实体（例如，将&lt;替换为&lt;）。
...
可以构建一个模板或变量库，以便在多个样式表之间共享。 “longMonth” **模板**&#x200B;如上所示，是在样式表中远程定位模板以便以后可以重复使用的好处的典型示例。
...
在HTML输出文档中输入的图像可使用绝对或相对参照进行参照。

相对引用允许您输入包含图像的URL **NcmResourcesDir** 和 **NcmResourcesDirPreview** 选项。 这些选项包含要在Adobe Campaign客户端控制台中发布和预览的图像的位置。
...
在样式表处理期间， **_resPath** 根据上下文（预览或发布），输入XML文档的主元素上的属性会自动以一个或多个选项填充。
...
您还可以使用 **[!UICONTROL Public resources]** 根据在部署向导中输入的实例设置，声明映像并将其上传到服务器。
...
要以所需格式显示日期，Adobe Campaign提供 **格式日期** 函数作为输入，输入日期的内容以及使用以下语法指定输出格式的字符串： **%4Y/%2M/%2D %2H%2N%2S**
...
在XSLT语法中没有标准日期管理函数。 要以所需格式显示日期，Adobe Campaign提供了外部功能 **日期格式**. 此函数将日期的内容和一个字符串作为输入，该字符串使用以下语法指定输出格式： **%4Y/%2M/%2D %2H%2N%2S**

示例:

* 若要在中显示日期 **01/10/2018** 格式：
