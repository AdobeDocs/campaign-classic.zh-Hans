...
在迁移期间，从架构定义重新生成&#x200B;**NmsRecipient**&#x200B;表。 在Adobe Campaign之外对此表的SQL结构所做的任何更改都将丢失。

要检查的元素示例：

* 如果您已将列（或索引）添加到&#x200B;**NmsRecipient**&#x200B;表，但尚未在架构中对其进行详述，则不会保存该列。
* 默认情况下，**tablespace**&#x200B;属性会取回其值，换言之，取回部署向导中定义的值。
* 如果您已将引用视图添加到&#x200B;**NmsRecipient**&#x200B;表，则必须在迁移前将其删除。


## 迁移前 {#before-the-migration}

迁移到Adobe Campaign v7时，必须配置以下元素。 在启动&#x200B;**升级后**&#x200B;之前，必须解决这些元素。
...
* 镜像页面
  ...
  但是，作为临时解决方法（并且镜像页面仍处于活动状态），您可以返回旧个性化块以避免此问题，方法是更改选项&#x200B;**[!UICONTROL XtkAcceptOldPasswords]**&#x200B;并将其设置为&#x200B;**[!UICONTROL 1]**。 这不会影响新的v6.x个性化块的使用。

* 语法

  如果您在升级后遇到任何与语法相关的错误，则必须临时激活&#x200B;**serverConf.xml**&#x200B;文件中的&#x200B;**allowSQLInjection**&#x200B;选项，因为这样您就有时间重写代码。 在修改代码后，请确保重新激活安全性。
...
* Web应用程序
  ...
  升级后，如果在连接到识别的Web应用程序时遇到问题，则必须激活&#x200B;**serverConf.xml**&#x200B;文件中的&#x200B;**allowUserPassword**&#x200B;和&#x200B;**sessionTokenOnly**&#x200B;选项。 为避免出现任何安全问题，在解决问题后必须重新激活这两个选项。
