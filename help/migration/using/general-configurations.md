...
在v5中，**sessiontoken**&#x200B;参数在客户端（概述类型屏幕列表、链接编辑器等）和服务器端（Web应用程序、报告、jsp、jssp等）都有效。 在v7中，它仅在服务器端工作。 如果要像v5一样返回全部功能，则必须使用此参数修改链接，并通过连接页面传递：
...
未知的SQL函数调用不再自然发送到服务器。 当前，所有SQL函数都必须添加到&#x200B;**xtk:funcList**&#x200B;架构中（有关详细信息，请参阅[此部分](../../configuration/using/adding-additional-sql-functions.md)）。 在迁移时，将在升级后期间添加一个选项，该选项允许您保持与旧的未声明SQL函数的兼容性。 如果要继续使用这些函数，请检查是否确实在&#x200B;**节点级别定义了** XtkPassUnknownSQLFunctionsToRDBMS **[!UICONTROL Administration > Platform > Options]**&#x200B;选项。
...
如果您使用匿名JSSP，则必须在JSSP （**文件）的中继规则中添加** httpAllowed=&quot;true&quot;**[!UICONTROL serverConf.xml]**&#x200B;参数：
...
对于所有其他对象类型，语法现在为&#x200B;**[!UICONTROL myObject`[`&quot;attribute&quot;`]`]**。 例如，使用以下语法的非XML对象现在必须使用以下语法： **[!UICONTROL employee.@sn]**&quot;sn&quot;**[!UICONTROL employee`[`。`]`]**
...
在v7中，已移动选件内容。 在v6.02中，内容位于每个呈现模式(**nms:emailOfferView**)中。 在v7中，内容现在位于选件架构中。 因此，在升级后，该内容在界面中将不可见。 升级后，必须重新创建选件内容，或开发一个脚本，以自动将内容从呈现架构移至选件架构。
...
1. 在&#x200B;**[!UICONTROL Type of Environment]**&#x200B;选项卡的&#x200B;**[!UICONTROL General]**&#x200B;字段中，选择&#x200B;**[!UICONTROL Live]**。
...
-->
如果您希望从新的报告功能中获益，则必须重新发布报告。 在这种情况下，请检查所有脚本，并在必要时进行更改。 关于PDF导出，如果您为Open Office添加了特定脚本，则新PDF导出引擎(PhantomJS)将不再能支持该脚本。
