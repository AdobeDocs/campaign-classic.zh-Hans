...
您可以直接在Adobe Campaign控制台中执行翻译（请参阅[在编辑器](#managing-translations-in-the-editor)中管理翻译），或导出和导入字符串以将翻译外部化（请参阅[将翻译外部化](#externalizing-translation)）。

默认情况下，可用的翻译语言列表详见[更改表单显示语言](#changing-forms-display-language)。
...
要定义一种或多种翻译语言，请单击Web 应用程序的&#x200B;**[!UICONTROL Properties]**&#x200B;按钮，然后单击&#x200B;**[!UICONTROL Localization]**&#x200B;选项卡。 单击&#x200B;**[!UICONTROL Add]**&#x200B;按钮以定义Web 应用程序的新翻译语言。
...
为Web 应用程序添加翻译语言时（或当默认语言和编辑语言不同时）,**[!UICONTROL Translation]**&#x200B;子选项卡会添加到&#x200B;**[!UICONTROL Edit]**&#x200B;选项卡以管理翻译。
...
第一次打开此选项卡时，它不包含任何数据。 单击&#x200B;**[!UICONTROL Collect the strings to translate]**&#x200B;链接以更新Web 应用程序中的字符串。

Adobe Campaign收集所有静态元素的&#x200B;**[!UICONTROL Texts]**&#x200B;选项卡中定义的字段和字符串标签：HTML块、Javascript等 静态元素详见网络表单](../../web/using/static-elements-in-a-web-form.md)中的[静态元素。
...
> 
>如果系统词典中显示某些翻译缺失的警告，请参阅[转换系统字符串](#translating-the-system-strings)。

每次翻译字符串时，其翻译都会添加到翻译词典中。

当集合进程检测到已存在转换时，此转换将显示在字符串的&#x200B;**[!UICONTROL Text]**&#x200B;列中。 字符串的状态将变为&#x200B;**[!UICONTROL Translated]**。

对于从未翻译过的字符串，**[!UICONTROL Text]**&#x200B;字段为空，状态为&#x200B;**[!UICONTROL To translate]**。

### 筛选字符串{#filtering-strings}

默认情况下，显示Web 应用程序的每种翻译语言。 有两种默认过滤器:语言和状态。 单击&#x200B;**[!UICONTROL Filters]**&#x200B;按钮，然后单击&#x200B;**[!UICONTROL By language or status]**&#x200B;以显示匹配的下拉框。 您还可以创建高级过滤器。 有关详细信息，请参见[此页面](../../platform/using/creating-filters.md#creating-an-advanced-filter)。
...
1. 在下部输入其翻译。 要批准此选项，请选中&#x200B;**[!UICONTROL Translation approved]**&#x200B;选项。
...
## 将翻译外置{#externalizing-translation}
...
1. 选择&#x200B;**[!UICONTROL Export strategy]** :
   ...
   * **[!UICONTROL One file per language and per Web application]**:导出将生成多个文件。每个文件都将包含一个Web应用程序的翻译语言。
...
1. 然后选择要录制文件的&#x200B;**[!UICONTROL Target folder]**。
...
>[!NOTE]
>
>导出文件的名称会自动生成。 如果多次执行相同的导出操作，您将用新文件替换现有文件。 如果需要保留以前的文件，请更改&#x200B;**[!UICONTROL Target folder]**，然后再次单击&#x200B;**[!UICONTROL Start]**&#x200B;以运行导出。

当您以&#x200B;**CSV格式**&#x200B;导出文件时，每种语言都链接到状态和批准状态。 **批准？** 列允许您批准转换。此列可能包含值&#x200B;**Yes**&#x200B;或&#x200B;**No**。 对于集成编辑器（请参阅在编辑器[中管理翻译），批准翻译是可选的，不会阻止该过程。](#managing-translations-in-the-editor)
...
Web表单以Web应用程序属性的&#x200B;**[!UICONTROL Localization]**&#x200B;选项卡中指定的默认语言显示。 要更改语言，必须将以下字符添加到URL的末尾（其中&#x200B;**xx**&#x200B;是语言的符号）:
...
**默认系统词典**:某些语言包括默认词典，其中包含系统字符串的翻译。有关详细信息，请参阅[转换系统字符串](#translating-the-system-strings)。
...
以下Web表单提供四种语言：英语、法语、德语和西班牙语。 所有字符串都已通过Web表单的&#x200B;**[!UICONTROL Translation]**&#x200B;选项卡进行翻译。 由于默认语言是英语，因此在发布调查时，请使用标准URL以英语显示调查。
...
系统字符串是所有Web应用程序使用的开箱即用的字符串。 例如：**[!UICONTROL Next]**、**[!UICONTROL Previous]**、**[!UICONTROL Approve]**&#x200B;按钮、**[!UICONTROL Loading]**&#x200B;消息等。 默认情况下，某些语言包含包含这些字符串的翻译的词典。 语言列表在[更改表单显示语言](#changing-forms-display-language)中有详细介绍。
...
1. 在窗口的上部，选择要转换的系统字符串，然后单击下部的&#x200B;**[!UICONTROL Add]**。
...
1. 单击&#x200B;**[!UICONTROL Add]**&#x200B;按钮，然后输入图像的&#x200B;**[!UICONTROL Internal name]**、**[!UICONTROL Label]**&#x200B;和标识符（标志）。 要添加新映像，请联系您的管理员。
