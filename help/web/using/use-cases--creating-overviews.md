...
1. 将光标放在 **[!UICONTROL Page]** Web应用程序的节点并添加 **[!UICONTROL Select folder]** 元素(**[!UICONTROL Advanced controls > Select folder]**)。
1. 在 **[!UICONTROL Storage]** 窗口打开时，单击 **[!UICONTROL Edit variables]** 链接。
1. 根据需要更改变量标签。
1. 将变量名称更改为 **文件夹** 值。

   >[!NOTE]
   >
   >变量的名称必须与链接到文件夹（在架构中定义）的元素的名称匹配，即 **文件夹** 这种情况下。 引用表时必须重复使用此名称。
...
1. 将光标放在列表中，并放在 **[!UICONTROL Advanced]** 选项卡中，引用之前在中创建的变量 **[!UICONTROL Folder filter XPath]** 选项卡中。 您必须使用文件夹链接涉及的元素的名称，即 **文件夹**.

   ![](assets/s_ncs_configuration_webapp_variable002.png)

   >[!NOTE]
   >
   >在此阶段，Web应用程序不在其应用程序上下文内，因此无法在文件夹上测试过滤器。
...
在概览中配置列表时，可以选择为列表中的每个条目显示附加详细信息。
...
在此部分中，您将创建一个过滤器，用于显示由特定操作员创建的Web应用程序的概述。 此过滤器使用链接编辑器创建。 选择运算符后，请刷新列表以应用筛选器；这需要创建刷新链接。
...
1. 将光标放在上一个阶段创建的容器上，并通过 **[!UICONTROL Advanced controls]** 菜单。
