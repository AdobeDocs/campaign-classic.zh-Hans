...
Adobe Campaign附带 **nlserver** 包，其中包含给定版本的二进制文件和配置文件。

安装命令使您能够：

* 将文件复制到 **/usr/local/neolane**
* 创建一个Adobe Campaign Linux帐户（以及关联的组），该帐户创建于 **/usr/local/neolane** 作为其主目录
* 创建自动脚本 **/etc/init.d/nlserver6** 在启动时使用，或创建一个系统单元

>[!NOTE]
>
>此 **neolane** 在运行命令之前不能创建系统用户。 此 **neolane** 用户是在安装期间自动创建的。
>
>此 **home** 目录链接到 **neolane** 用户也会在中自动创建 **[!UICONTROL /usr/local/neolane]**. 请确保上面有足够的空间 **[!UICONTROL /usr/local]** 磁盘。
...
此 `bc` 命令，执行 [网络报告](../../production/using/monitoring-processes.md#automatic-monitoring-via-adobe-campaign-scripts)默认情况下，并非所有Linux分发版本上都提供。 要检查该命令是否可用，请运行 `which bc` 命令。 如果没有，则必须安装它。
...
如果是首次执行安装，则 **customer.sh** 文件可能尚未在服务器上存在。
...
* 对于Java开发工具包(JDK)：
  ...
  默认情况下，Adobe Campaign环境的配置脚本(`~/nl6/env.sh`)搜索JDK安装目录。 但是，建议指定需要使用的JDK。 为此，您可以强制 **JDK_HOME** 环境变量：
