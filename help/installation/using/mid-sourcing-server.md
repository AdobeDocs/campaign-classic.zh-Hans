...
product: campaign
title: 在活动中安装中间源服务器
description: 本节详细介绍在活动中安装和配置中间源服务器
...
本节详细介绍了中间源服务器的安装和配置，以及允许第三方以&#x200B;**中间源**&#x200B;模式发送消息的实例的部署。

“中间源”体系结构显示在[中间源部署](../../installation/using/mid-sourcing-deployment.md)中。

安装中间源服务器与正常安装服务器的过程相同（请参阅标准配置）。 它是一个独立实例，具有自己的数据库，可用于运行投放。 简而言之，它包含一个额外的配置，允许远程实例在中间源模式下通过它执行投放。
...
以下过程使用单个中间源服务器详细描述配置。 还可以使用多台服务器。 同样，也可以从内部配置发送某些消息（例如，工作流通知）。

### 安装和配置中间源服务器以进行应用程序部署{#installing-and-configuring-the-application-server-for-mid-sourcing-deployment}
...
* 在步骤&#x200B;**12**&#x200B;和&#x200B;**13**&#x200B;期间，需要在连接URL中指示8080端口（因为控制台直接与Tomcat通信，而不是通过Web服务器通信）。 URL将变为[http://console.campaign.net:8080](http://console.campaign.net)。 在步骤&#x200B;**13**&#x200B;中，选择&#x200B;**[!UICONTROL Issue towards Mid-sourcing]**&#x200B;包以及要安装的包。
   ...
   >[!CAUTION]
   >
   >技术投放的默认路由会自动替换为通过中间源发送的电子邮件路由。

### 安装和配置中间源服务器{#installing-and-configuring-the-mid-sourcing-server}

在客户端控制台中，找到使用中间源&#x200B;**路由帐户的**&#x200B;电子邮件中间源(在&#x200B;**/Administration/外部帐户/**&#x200B;文件夹中)。 将服务器&#x200B;**、** account **、** password **和**&#x200B;镜像页面URL **的** URL设置填充为托管中间源服务器的服务器提供商提供的信息。 测试连接。

>[!NOTE]
>
>**mid-sourcingEmitter**&#x200B;选项创建两个&#x200B;**中间源**&#x200B;工作流。 默认情况下，该进程每1小时20分钟运行一次，并在中间源服务器上收集投放信息。
...
1. 安装应用程序服务器：
   ...
   >[!CAUTION]
   >
   >如果您安装中间源服务器并想安装其他Adobe Campaign模块，我们建议使用投放模块，而不是活动模块。
...
1. 用于在中间源模式下接收的配置
   ...
   设置提交帐户密码：在&#x200B;**/中间源/Access Management/Operators/**&#x200B;文件夹中，远程实例使用&#x200B;**mid**&#x200B;运算符以中间源模式提交。 您必须为此运算符设置口令，并将其授予提交实例的管理员。
...
中间源实例可能由多个提交实例共享。 这些实例中的每个实例都需要与中间源数据库中的运算符关联。 要在中间源服务器上创建第二个帐户，请执行以下操作：

1. 在&#x200B;**[!UICONTROL Mid-sourcing > Deliveries]**&#x200B;节点中创建一个将与默认中间源帐户关联的文件夹(例如：prod)。
1. 在&#x200B;**[!UICONTROL Mid-sourcing > Deliveries]**&#x200B;节点中创建与帐户同名的文件夹(例如：acceptance_test)。
...
1. 选择&#x200B;**[!UICONTROL Restrict to data in the sub-folders of]**&#x200B;选项，然后选择投放文件夹，以将此运算符限制到中间源投放文件夹。

   ![](assets/mid_recette_user_restrictions.png)

1. 使用以下命令重新启动Web模块：**nlserver重新启动web**。

必须更改serverConf.xml文件中的中间源服务器设置。 必须在现有行的“IP地址关联管理”部分中添加以下行：
...
**&#39;marketing_account_operator_name&#39;。&#39;关联_name&#39;.&#39;关联_group&#39;**

“marketing_account_operator_name”与在中间源实例中声明的中间源帐户的内部名称相关。

“关联_name”与给予关联的任意名称有关。 此名称必须唯一。 授权字符为`[a-z]``[A-Z]``[0-9]`。 其目的是声明一组公共IP地址。

“关联_group”与在每个投放中使用的目标映射中声明的子关联相关。 最后一部分包括“。” 如果没有子关联，则忽略。 授权字符为`[a-z]``[A-Z]``[0-9]`。
...
1. 如有必要，请指定存储投放中包含的图像的位置。 为此，请从下拉列表中选择发布模式之一。
