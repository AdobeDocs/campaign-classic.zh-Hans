...
使用Campaign [联合数据访问](../../installation/using/about-fda.md) (FDA)用于处理存储在外部数据库中的信息的选项。 按照以下步骤配置对Teradata的访问权限。
...
您需要安装Teradata的驱动程序，才能实现与Campaign的连接。
...
## 外部帐户Teradata{#teradata-external}

使用Teradata外部帐户可将Campaign实例连接到Teradata外部数据库。
...
| Option | 说明 |
|---|---|
| TD_MAX_SESSIONS | 指定Teradata并行传输程序可以为操作员作业获取的最大登录会话数。 <br>[有关更多信息，请参阅此页面](https://documentation.sas.com/doc/en/pgmsascdc/9.4_3.5/ds2ref/p1naft0um1kn3vn1ubgkrjdf7c3a.html). |
| 时区名称 | 服务器时区的名称。 |
| 字符集 | 用于配置Teradata字符集。 <br>[有关更多信息，请参阅此页面](https://docs.teradata.com/r/ODBC-Driver-for-Teradata-User-Guide/May-2017/Configuration-of-odbc.ini-in-UNIX/Linux-and-Apple-OS-X/Teradata-DSN-Options#rub1478609534082__table_N102D3_N102B6_N102B3_N10001). |
...
teradata驱动程序提供了自己的ODBC库，但此库可能与其他ODBC外部帐户不兼容。

如果要配置另一个也使用ODBC的外部帐户(例如Snowflake)，则需要将一个ODBCLib选项集添加到默认ODBC库的path (`/usr/lib/x86_64-linux-gnu/libodbc.so` 关于Debian和 `/usr/lib64/libodbc.so` 在RHEL/CentOS上)。
...
配置此选项后，每次Campaign用户在Teradata数据库上执行查询时，Adobe Campaign都将发送与此用户关联的元数据，其中包含一个键列表。 然后，Teradata管理员可以使用此数据来进行审核或管理访问权限。
...
1. 使用  **[!UICONTROL Default]** 输入在用户没有关联查询范围时将使用的默认查询范围。 如果此字段留空，则没有查询范围的用户将无法使用Teradata。

1. 使用 **[!UICONTROL Users]** 用于为每个用户指定查询频带的字段。 您可以根据需要添加任意数量的键/值对，例如，priority=1；workload=high。 如果用户未分配查询范围，则 **[!UICONTROL Default]** 字段将被应用。

1. 查看 **[!UICONTROL Active]** 用于激活此功能的框

#### 外部帐户疑难解答 {#external-account-troubleshooting}

如果在测试连接时出现以下错误 **TIM-030008日期“2”：缺少字符(iRc=-53)** 确保正确安装了ODBC驱动程序，并为Campaign服务器设置了LD_LIBRARY_PATH (Linux) / PATH (Windows)。

错误 **ODB-240000 ODBC错误： [Microsoft][ODBC Driver Manager] 未找到数据源名称，未指定默认驱动程序。** 使用16.X驱动程序时在Windows中发生。 Adobe Campaign要求odbcinst.ini中的teradata名为“{teradata}”。

* 从Campaign 18.10开始，您可以在外部帐户的选项中添加ODBCDriverName=&quot;Teradata数据库ODBC驱动程序16.10&quot;。 版本号可以更改，通过运行odbcad32.exe并访问“驱动程序”选项卡，可以找到确切的名称。

* 如果您使用的是较低版本的Campaign，则必须将驱动程序安装创建的odbcinst.ini的Teradata部分复制到名为Teradata的新部分。 在这种情况下，可以使用Regedit。 如果您的基数是latin1，则必须添加 **APICharSize=1** 选项中。
...
Adobe Campaign不会对其将在数据库中创建的对象设置保护模式（回退）。 您可能需要在Adobe Campaign将用来使用以下查询连接到Teradata数据库的用户上设置默认值：
...
如果要在Adobe Campaign实例中使用md5函数，则必须通过此在Teradata数据库上安装用户模式函数 [页面](https://downloads.teradata.com/download/extensibility/md5-message-digest-udf) (md5_20080530.zip)。
...
如果要在Adobe Campaign实例中使用udf_utf16to8函数，则必须从以下位置在Teradata数据库上安装用户模式函数： **teradataunicode工具包** 的 [页面](https://downloads.teradata.com/download/tools/unicode-tool-kit) (utk_release1.7.0.0.zip)。

下载文件的sha1如下e58235f434f52c71316a577cb48e20b97d24f470。
...
* teradataODBC驱动程序，可在此找到 [页面](https://downloads.teradata.com/download/connectivity/odbc-driver/linux)

* teradata工具和实用程序（用于批量加载），可在此处找到 [页面](https://downloads.teradata.com/download/tools/teradata-tools-and-utilities-linux-installation-package-0)

文件名和sha1：

* tdodbc1620__linux_indep.16.20.00.00-1.tar.gz 121fdd978b56fe1304fc5cb7819741b0847f44fd

* TeradataToolsAndUtilitiesBase__linux_indep.16.20.01.00.tar.gz b 29d0af5ffd8dcf68a9dbbba6f8639387b19c563

如果Linux分发版没有软件包，您可以按照CentOS 7上的说明进行安装（例如使用docker），然后在Adobe Campaign服务器上复制/opt/teradata的内容。
...
确保安装ODBC驱动程序和Teradata并行传输程序库。 它将安装用于在Teradata数据库上进行批量加载的telapi.dll。

确保驱动程序和实用程序的路径位于nlserver在执行期间将具有的PATH变量中。 默认情况下，该路径为C:\Program Files (x86) \Teradata\Client\15.10\bin （在Windows 32位上）或C:\Program Files\Teradata\Client\15.10\bin （在64位上）。

## 时区 {#timezone}

teradata使用非标准的时区名称，您可以在 [teradata站点](https://docs.teradata.com/reader/rgAb27O_xRmMVc_aQq2VGw/oGKvgl7gCeBMTGrp59BnwA). Adobe Campaign将尝试将外部配置中给定的时区转换为Teradata所了解的情况。 如果找不到通信，则会找到会话的最近的GMT+X（或GMT-X）时区，日志中会显示警告。

完成转换后，读取了名为teradata_timezones.txt的文件，该文件应位于linux下的/usr/local/neolane/nl6/datakit目录下。 如果您编辑此文件，请确保联系Adobe Campaign团队以在源代码中进行更改，否则此文件将在下次Campaign更新时覆盖。
...
在Teradata文档中使用批量加载或“快速加载”时，Campaign无法指示时区。 因此，建议设置Campaign将用来连接的用户的默认时区：
