...
Adobe Campaign的总体配置在位于安 **装目录的conf** 目录中的serverConf **** .xml文件中定义。 本节列表serverConf.xml文件的所有不 **同节点和参数** 。
...
以下是dataStore节点的不 **同参数** 。 这是定义服务器数据源的位置。
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> 朗<br /> </td> 
   <td> 实例语言(明细列表)。 可能的值为“fr_FR”(Français)、“en_GB”（英国）、“en_US”（美国）、“de_DE”(Deutsch)和“ja_JP”（日语）。<br /> </td> 
   <td> 字符串<br /> </td> 
   <td> 'en_US'<br /> </td> 
  </tr> 
  <tr> 
...
以下是dataStore > proxyAdjust节 **点的不同参数** 。 与常规表达式匹配的URL将根据urlBase中定义的URL重新生成。
...
在“数 **据存储”>“数据源”>** sqlParams节点中，配置SQL参数：
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> maxCnx<br /> </td> 
   <td> 拒绝新连接前允许的最大连接数。 看看这 <a href="https://helpx.adobe.com/campaign/kb/how-to-increase-the-maximum-number-of-database-connections-from-.html">个技术</a>。<br /> </td> 
   <td> 短<br /> </td> 
  </tr> 
  <tr> 
...
以下是dataStore > preposerCommand节 **点的不同参数** 。 这些是用于预处理“加载文件”工作流活动的授权命令。
...
以下是exec（命令执行）节 **点的** 不同参数。

有关其他信息，请参阅限 [制已授权的外部命令](../../installation/using/configuring-campaign-server.md#restricting-authorized-external-commands)。
...
以下是htmlToPdf节点的 **不同参数** 。 这是将网页转换为PDF文档的服务配置。
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> Max。 在一台计算机上一次允许的转换进程数。<br /> </td> 
   <td> 长<br /> </td> 
   <td> 5<br /> </td> 
  </tr> 
  <tr> 
...
以下是javaScript节点的不 **同参数** 。 这是JavaScript解释器的配置。

有关其他信息，请参阅 [报告文档](../../reporting/using/actions-on-reports.md#memory-allocation) 和此 [技术说明](https://helpx.adobe.com/campaign/kb/out-of-memory-error-in-js-code-activity-in-workflows.html)。
...
以下是模块节点的不 **同参** 数。 这是命名空间限制模块xtk的配置。
...
以下是ooconv节点的不 **同参数** 。 这是文档转换服务器的配置。
...
有关其他信息，请参 [阅代理连接配置](../../installation/using/configuring-campaign-server.md#proxy-connection-configuration)。
...
在proxyConfig > **HTTP代理／安全代理节点中** ，配置以下参数。
...
以下是urlPermission节点的不 **同参数** 。 这是Javascript代码可以访问的URL的列表。

列表域和常规表达式，指定Javascript代码中遇到的URL是否可由Adobe Campaign服务器使用。

如果找不到URL，则根据指定的默认模式执行默认操作。

有关其他信息，请参 [阅传出连接保护](../../installation/using/configuring-campaign-server.md#url-permissions)。
...
<table> 
 ...
 <tbody> 
  <tr> 
   <td> 行动<br /> </td> 
   <td> 如果URL不在授权列表(明细列表)，则执行默认操作。 可能的值为“ignore”（无警告消息授权，这需要禁用保护）、“warn”（授权并发出警告消息）和“deny”（禁止访问URL）。<br /> </td> 
   <td> 字符串<br /> </td> 
   <td> 拒绝<br /> </td> 
  </tr> 
  <tr> 
...
如果记录满足 **dnsSuffix** ，但 **不满足urlRegEx**，将检查以下记录。
...
下面是存档节点的不 **同参** 数。 这是在后台执行的存档操作的配置。

有关其他信息，请参 [阅激活电子邮件归档（内部部署）](../../installation/using/email-archiving.md#activating-email-archiving--on-premise-)。
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 进程自动重新启动的一天的时间。 请参 <a href="../../installation/using/configuring-campaign-server.md#automatic-process-restart" target="_blank">阅自动进程重启</a>。<br /> </td> 
   <td> 字符串<br /> </td> 
   <td> '06:00:00' <br /> </td> 
  </tr> 
  <tr> 
   <td> purgeArchivesDelay<br /> </td> 
   <td> 删除未处理的电子邮件的天数。<br /> </td> 
   <td> 长<br /> </td> 
   <td> 7<br /> </td> 
  </tr> 
  <tr> 
...
以下是inMail节点的不 **同参数** 。 这是入站电子邮件管理模块的配置。
...
在inMail > **msgDump节点** ，配置以下参数。 这是已处理消息的转储配置。
...
下面是interactiond节点的不 **同参** 数。 这是入站交互事件的写入守护程序配置。

有关其他信息，请参 [阅交互——数据缓冲区](../../installation/using/interaction---data-buffer.md)。
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> Max。 共享内存中存储的用于呼叫数据的字符数。<br /> </td> 
   <td> 长<br /> </td> 
   <td> 0<br /> </td> 
  </tr> 
  <tr> 
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 要考虑的连接阈值。 如果errorPeriodSec指定期间的连接总数严格低于阈值，则不为给定路径生成错误统计信息。<br /> </td> 
   <td> 长<br /> </td> 
   <td> 100<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 启用签名机制。 这提高了跟踪电子邮件链接的安全性。<br /> </td> 
   <td> 布尔值<br /> </td> 
   <td> 真<br /> </td> 
  </tr>
  <tr> 
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> 要传送的作业的数据库轮询频率。 此值指示数据库轮询频率（以秒为单位）。 为获得等待投放的作业列表,MTA会定期轮询数据库。 当没有作业等待时，轮询周期由此值定义。 否则，如果作业已被转移到子服务器，则此轮询持续时间将自动缩短为1秒，以便新作业能够尽快被再次处理，即当子服务器再次可用时。 这并不意味着在子服务器再次可用之前，每秒钟都会执行查询库操作。 事实上，数据库访问仅在至少一个子服务器可用时才进行。<br /> </td> 
   <td> 长<br /> </td> 
   <td> 30<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 子服务器的最大数量。 表示正在运行的最大服务器数。 建议将此数量限制在与服务器内存资源兼容的最佳值。 可以在投放期间检查此项。 使用的内存不应超过可用物理内存的三分之一，否则将使用交换。 请参 <a href="../../installation/using/configuring-campaign-server.md#mta-child-processes" target="_blank">阅MTA子进程</a>。<br /> </td> 
   <td> 长<br /> </td> 
   <td> 2<br /> </td> 
  </tr> 
  <tr> 
   <td> minSpareServers<br /> </td> 
   <td> 最少子服务器数。 MTA会尝试至少保持此数量的服务器运行。 如果数量较少，则每秒重新启动新服务器，直到达到此值。<br /> </td> 
   <td> 长<br /> </td> 
   <td> 0<br /> </td> 
  </tr> 
  <tr> 
...
有关其他信息，请参阅电 [子邮件发送优化](../../installation/using/email-deliverability.md#email-sending-optimization)。
...
在mta > **子> smtp节点中** ，配置以下参数。 这是SMTP会话的配置。
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 空闲会话超时。 此参数仅在会话被重用以向给定域发送多个消息时才使用。 当MTA完成消息传输时，它使用的SMTP会话不会被系统关闭。 如果消息已准备好发送到此同一域，则将重用同一SMTP会话，这就是会话不自动关闭的原因。 通过参数IdleSessionTimeout参数，可以定义SMTP会话在等待另一条消息时保持活动状态的时间。 持续时间一经过，会话即自动关闭。<br /> </td> 
   <td> 长<br /> </td> 
   <td> 5<br /> </td> 
  </tr> 
  <tr> 
...
在mta > **子级> smtp > IPAffinity节点** ，配置以下参数。 这是对具有IP地址的关联进行管理的配置，以优化传出SMTP通信。

有关详细信息，请参 [阅列表IP地址以使用和](../../installation/using/email-deliverability.md#list-of-ip-addresses-to-use)[管理出站SMTP通信和关联](../../installation/using/configuring-campaign-server.md#managing-outbound-smtp-traffic-with-affinities)。
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 包含的以逗号分隔的列表域掩码。<br /> </td> 
   <td> 字符串<br /> </td> 
  </tr> 
  <tr> 
   <td> excludeDomains<br /> </td> 
   <td> 要排除的以逗号分隔的列表域掩码。<br /> </td> 
   <td> 字符串<br /> </td> 
  </tr> 
  <tr> 
   <td> heloHost<br /> </td> 
...
以下是nmac > relay节 **点的不同参数** 。 这将配置消息投放（ios http2连接器）的中继的使用。
...
下面是管道节点的不 **同参** 数。 这是管道服务事件处理模块的配置。
...
有关其他信息，请参阅 [定义安全区](../../installation/using/configuring-campaign-server.md#defining-security-zones)。
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> 使用Netsize建立连接时的超时（以秒为单位）。<br /> </td> 
   <td> 长<br /> </td> 
   <td> 30<br /> </td> 
  </tr> 
 </tbody> 
...
以下是syslogd节点的不 **同参数** 。 这是日志管理模块的配置。
...
以下是Web > jsp节 **点的不同参数** 。 这是JSP使用的参数配置。
...
以下是web > jssp节 **点的不同参数** 。 这是JSSP使用的参数配置。
...
<table> 
 ...
 <tbody> 
  <tr> 
   <td> debugRelay<br /> </td> 
   <td> 在调试模式下开始Web服务器中的HTTP中继模块。<br /> </td> 
   <td> 布尔值<br /> </td> 
   <td> 假<br /> </td> 
  </tr> 
  <tr> 
...
为每个 **URL添加一个Web >** relay > url节点（插入顺序定义优先级），并使用以下参数。

有关其他信息，请参 [阅动态页面安全和中继](../../installation/using/configuring-campaign-server.md#dynamic-page-security-and-relays)[和部分](../../installation/using/deploying-an-instance.md#synchronizing-public-resources)。
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> status<br /> </td> 
   <td> 公共资源(明细列表)的同步状态。 可能的值为“normal”（正常执行）、“blacklist”（在出现错误404时添加为的url）和“spare”（如果存在，则文件上传到备用服务器）。<br /> </td> 
   <td> 字符串<br /> </td> 
   <td> 正常<br /> </td> 
  </tr> 
  <tr> 
...
为每个 **HTTP头添加一个Web > relay > responseHeader节点** ，以添加转发到中继的回复。
...
以下是Web > spamCheck节 **点的不同参数** 。 这是电子邮件防垃圾邮件评分评估参数的配置。

有关其他信息，请参 [阅配置SpamAssassin](../../installation/using/configuring-spamassassin.md)。
