...
使用Campaign **联合数据访问** (FDA)用于处理存储在外部数据库中的信息的选项。 按照以下步骤配置对的访问权限 [!DNL Snowflake].
...
1. 安装ODBC驱动程序后，需要重新启动Campaign Classic。 要执行此操作，请运行以下命令：
...
1. 配置 **[!UICONTROL Snowflake]** 外部帐户身份验证：
   ...
   * 对于密钥对验证，单击 **[!UICONTROL Keypair Auth]** 选项卡，以使用 **[!UICONTROL Private key]** 验证并复制并粘贴您的 **[!UICONTROL Private key]**.
...
| bulkthreads | 用于Snowflake批量加载器的线程数，线程越多，批量加载越大，性能越好。 默认设置为1。 根据计算机线程数，可调整该数量。 |
| chunkSize | 确定批量加载程序块的文件大小。 默认设置为128MB。 可以修改以获得更优化的性能（当与bulkThreads一起使用时）。 更多并发活动线程意味着更好的性能。 <br>有关更多信息，请参阅 [Snowflake文档](https://docs.snowflake.net/manuals/sql-reference/sql/put.html). |
