...
1. 创建或循环使用数据库：如果要创建新数据库或重用现有数据库，请选择此选项。 请参阅 [案例1：创建/回收数据库](#case-1--creating-recycling-a-database).
...
确定服务器并选择要执行的操作类型。 在这个案例中， **[!UICONTROL Create or recycle a database]**.

![](assets/s_ncs_install_db_oracle_creation01.png)

根据选定的数据库引擎，服务器标识信息可能会有所不同。

* 对于 **oracle** 引擎，填充 **TNS名称** 已为应用程序服务器定义。
* 对于 **PostgreSQL** 或 **DB2** 引擎，您必须指定在应用程序服务器上定义的DNS名称（或IP地址）才能访问数据库服务器。
...
* 指示数据库是否必须采用Unicode。
   ...
   >[!NOTE]
   >
   >对于Oracle数据库， **[!UICONTROL Unicode storage]** 选项允许您使用 **NCLOB** 和 **NVARCHAR** 键入字段。
   > 
...
请参阅您的许可协议，查看您有权安装的解决方案和选项，例如“互动”或“社交营销”。

![](assets/s_ncs_install_modules.png)

### 步骤5 — 创建步骤 {#step-5---creation-steps}

此 **[!UICONTROL Creation steps]** 窗口用于显示和编辑用于创建表的SQL脚本。
...
* 对于Oracle数据库，Adobe Campaign用户必须有权访问Oracle库，通常作为 **oinstall** 组。
* 此 **[!UICONTROL Set or change the administrator password]** 选项允许您输入链接到具有管理员权限的Adobe Campaign操作员的密码。

   出于安全原因，我们建议定义Adobe Campaign帐户管理员密码。
...
确定服务器并选择要执行的操作类型。 在这个案例中， **[!UICONTROL Use an existing database]**.
...
* 对于 **oracle** 引擎，填充 **TNS名称** 已为应用程序服务器定义。
* 对于 **PostgreSQL** 或 **DB2** 引擎，您必须指定在应用程序服务器上定义的DNS名称（或IP地址）才能访问数据库服务器。
* 对于 **Microsoft SQL Server** 引擎时，您必须定义：
...
请参阅您的许可协议，查看您有权安装的解决方案和选项，例如“互动”或“潜在客户”。
