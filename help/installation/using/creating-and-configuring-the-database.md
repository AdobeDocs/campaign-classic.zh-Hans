...
1. 使用现有数据库：如果管理员已创建空数据库并且您希望使用它，则选择此选项；或扩展现有数据库的结构。 请参阅[案例2:使用现有数据库](#case-2--using-an-existing-database)。
...
支持的活动库列在[兼容性矩阵](../../rn/using/compatibility-matrix.md)中。
...
为此，请输入具有访问数据库权限的&#x200B;**管理系统帐户**&#x200B;的名称和密码，即：
...
* 指示数据库是否必须位于Unicode中。
   ...
   使用&#x200B;**[!UICONTROL Unicode database]**&#x200B;选项，您可以在Unicode中存储所有字符类型，而不考虑语言。

   >[!NOTE]
   >
   >对于Oracle数据库，使用&#x200B;**[!UICONTROL Unicode storage]**&#x200B;选项可以使用&#x200B;**NCLOB**&#x200B;和&#x200B;**NVARCHAR**&#x200B;类型字段。
   > 
...
使用&#x200B;**[!UICONTROL Creation steps]**&#x200B;窗口可显示和编辑用于创建表的SQL脚本。

![](assets/s_ncs_install_db_oracle_creation04.png)

* 对于Oracle、Microsoft SQL Server或PostgreSQL数据库，管理员还可以定义创建存储库对象时使用的&#x200B;**数据参数**。

   这些参数接收确切的表空间名称(警告：区分大小写)。 它们分别存储在&#x200B;**[!UICONTROL Administration > Platform > Options]**&#x200B;节点中的以下选项中（请参见[本节](../../installation/using/configuring-campaign-options.md#database)）:
...
* 对于OracleAdobe Campaign库，用户必须具有访问Oracle库的权限，通常作为&#x200B;**oinstall**&#x200B;组的成员。
* 通过&#x200B;**[!UICONTROL Set or change the administrator password]**&#x200B;选项，您可以输入与具有管理员权限的Adobe Campaign操作员链接的口令。

   我们建议为安全目的定义Adobe Campaign帐户管理员密码。

### 第6步——创建数据库{#step-6---creating-the-database}

在向导的最后一个阶段，您可以创建数据库。 单击&#x200B;**[!UICONTROL Start]**&#x200B;进行确认。
...
* 对于Oracle、Microsoft SQL Server或PostgreSQL存储库，管理员可以定义创建数据库对象时使用的&#x200B;**参数**。
* 对于OracleAdobe Campaign库，用户必须具有访问Oracle库的权限，通常作为&#x200B;**oinstall**&#x200B;组的成员。
* 通过&#x200B;**[!UICONTROL Set or change the administrator password]**&#x200B;选项，您可以输入与具有管理员权限的Adobe Campaign操作员链接的口令。
