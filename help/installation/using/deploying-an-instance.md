...
退回由Adobe Campaign自动处理，应用&#x200B;**管理>营销活动管理>不可投放项管理>投放日志资格**&#x200B;节点中列出的规则。 有关详情，请参阅[退回邮件管理](../../delivery/using/delivery-failures-quarantine.md#bounce-mail-management)。
...
* 黑名单：如果URL返回404错误，则会将其添加到。 列入阻止列表中URL的持续时间（以秒为单位）由&#x200B;**timeout**&#x200B;属性定义，其默认值为60秒。
