...
* **[!UICONTROL Reply address text]** :该名称可自定义，当收件人单击其电子邮件客户端软 **[!UICONTROL Reply]** 件中的按钮时，将使用。
...
* **[!UICONTROL Exclude recipients who no longer wish to be contacted]** :选择此选项后，将不会联系阻止列表收件人,
...
对于标准安装，不 **得修改链接到URL** 搜索的参数。 有关所有其他参数，请与Adobe联系。
...
1. 发布选项。 For more information, refer to the [Publication modes](#publication-modes) section.
...
* 阻止列表:如果URL返回404错误，则会将该URL添加到阻止列表。 阻止列表中URL的持续时间（以秒为单位）由默认值 **为** 60s的超时属性定义。
