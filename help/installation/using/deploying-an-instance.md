...
>[!NOTE]
>
>对于由Adobe托管的部署，只能由Adobe执行服务器端配置。 要了解有关不同部署的更多信息，请参阅 [托管模型](../../installation/using/hosting-models.md) 区域或至 [此页面](../../installation/using/capability-matrix.md).
...
* **[!UICONTROL Namespace]** ：输入小写的短标识符。 目的是区分您的特定配置和工厂配置在升级时发生的情形。 默认命名空间为 **cus**  — 适用于客户。
...
* **[!UICONTROL Tracking]** ：启用目标群体跟踪（打开数和点击数）。 请参阅 [跟踪配置](#tracking-configuration).
...
* **[!UICONTROL Sender address]** ：输入发件人的电子邮件地址。 从Adobe Campaign发送电子邮件时， **发件人地址** 邮箱不受监视，营销用户无法访问此邮箱。 Adobe Campaign也不提供自动回复或自动转发此邮箱中接收的电子邮件的功能。 了解有关可投放性最佳实践的更多信息 [本文档内容](https://experienceleague.adobe.com/docs/deliverability-learn/deliverability-best-practice-guide/additional-resources/campaign/ac-starting-new-platform.html){_blank}.

* **[!UICONTROL Reply address text]** ：输入收件人单击 **[!UICONTROL Reply]** 按钮。
* **[!UICONTROL Reply address]** ：输入收件人单击 **[!UICONTROL Reply]** 电子邮件客户端软件中的URL按钮。 目的 **回复地址** 字段用于您希望收件人回复的地址与 **发件人地址**.  此地址必须是有效的电子邮件地址，链接到受监视的邮箱，并由客户托管。  例如，它可以是一个支持邮箱， `customer-care@customer.com`，可读取和响应电子邮件。

* **[!UICONTROL Error address]** ：输入包含错误的消息的电子邮件地址。 这是用于处理退回邮件的技术地址，包括Adobe Campaign服务器因目标地址不存在而收到的电子邮件。 此地址必须是有效的电子邮件地址，链接到受监视的邮箱，并由客户托管。 例如，它可能是退回邮箱， `errors@customer.com`. 您可以在投放或投放模板中更改此地址 **SMTP** 投放/投放模板属性的选项卡。 [了解详情](../../delivery/using/email-parameters.md#managing-bounce-emails-managing-bounce-emails)。
...
在Adobe Campaign数据库中，必须按如下方式创建所有电子邮件地址： `x@y.z`. 此 **x**， **y** 和 **z** 字符不得为空，并且不得包含非授权字符。

您可以在此处定义数据库的电子邮件字段中授权的字符（“数据策略”）。 当通过界面通过Web表单在数据库中输入信息以及导入数据时，列表未包含的字符将被禁止并因此被拒绝。

提供了两个列表： **仅限欧洲语言** 或 **仅限美国**. 如有必要，可以添加其他字符。

### 投放参数 {#delivery-parameters}

此 **高级参数……** 链接可让您访问提交选项，以及链接到重试和隔离的参数。
...
* **[!UICONTROL Delivery duration of messages]** ：在此时间之后，投放将停止（默认情况下，为5天）。
...
有关恢复的信息请参见 **恢复期** 和 **恢复次数** 字段：当无法访问收件人时，例如收件箱已满，默认情况下，程序将尝试联系收件人5次，每次尝试之间间隔一小时（在最大投放时间内）。 可以根据您的需求更改这些值。

>[!NOTE]
>
>对于托管或混合安装，如果已升级到 [增强MTA](../../delivery/using/sending-with-enhanced-mta.md)，不再使用Campaign重试参数。 软退回重试次数以及它们之间的时间长度由Enhanced MTA根据从消息的电子邮件域返回的退回响应的类型和严重性确定。
...
* **[!UICONTROL Label]** ：名称，其中包含下面提供的所有参数，
...
退回由Adobe Campaign自动处理，并应用中列出的规则 **管理> Campaign Management >无法投放项管理>投放日志资格** 节点。 有关详细信息，请参见 [退回邮件管理](../../delivery/using/understanding-delivery-failures.md#bounce-mail-management).
...
* **[!UICONTROL Address for errors]** ：填写此字段以仅将inMail进程不符合条件的错误消息传送到第三方地址。
...
此步骤仅在 **电子邮件渠道** 和 **跟踪** 将在部署向导的第一页中选择选项。 请参阅 [常规参数](#general-parameters).
...
* 使用在部署向导的此页面上输入的关于外部URL（无论是否安全）的信息来构建新URL。 除了此信息外，修改的链接还包含：投放的标识符、收件人和URL。
  ...
  Adobe Campaign在跟踪服务器上收集跟踪信息，以扩充收件人用户档案和与投放关联的数据( **[!UICONTROL Tracking]** 选项卡)。

  有关内部URL的信息仅供Adobe Campaign应用程序服务器用于联系跟踪服务器。

  有关详细信息，请参见 [跟踪服务器](#tracking-server).

* 配置URL后，您需要启用跟踪。 为此，必须在跟踪服务器上注册实例。

  有关详细信息，请参见 [正在保存跟踪](#saving-tracking).
...
1. **验证已为您预配置的跟踪实例**
   ...
   如果您没有的密码，则可以使用此选项。 **内部** 帐户；在这种情况下，会在跟踪服务器上为您预配置跟踪帐户。 输入重定向服务器的跟踪帐户的密码以验证跟踪实例。
...
此 **高级参数……** 通过链接，您可以访问重试和隔离选项。

![](assets/s_ncs_install_deployment_wiz_13.png)

有关重试的信息，请参见 **重试周期** 和 **重试次数** 字段：默认情况下，无法访问移动设备时，程序将重试5次，间隔至少15分钟（最长交付期）。 可根据您的需求对这些值进行调整。

隔离的配置选项如下所示：

* **[!UICONTROL Time between two significant errors]** ：输入默认值（默认为“1d”：天），以定义应用程序在递增错误计数器的失败之前等待的时间。
* **[!UICONTROL Maximum number of errors before quarantine]** ：达到此值后，将隔离该手机号码（默认情况下，“5”：出现第六次错误时，将隔离该号码）。 这意味着该联系人会自动从将来的投放中排除。
...
* **[!UICONTROL Consider all phone numbers as international ones]** ：如果选择此选项，应用产品会将国际格式应用于电话号码（国家/地区前缀是必填项，因为应用格式之前不会检查位数）。 如果未选中此选项，则必须在国际电话号码前面加上“+”或“00”。
* **[!UICONTROL Store all phone numbers using the international format]** ：此选项仅涉及 **国内** 导入或编辑的电话号码。 定义您要使用国内格式（如425 555 0150）还是国际格式（如+1 425 555 0150）
...
字段 **URL掩码** 用于指定自动上传图像时要跳过的URL掩码列表。 例如，如果您使用的图像存储在可从外部访问的站点上，尤其是在Internet站点上，则可以在此字段中输入站点URL。
...
* 跟踪服务器
  ...
  资源将自动复制到不同的跟踪服务器。 在步骤中配置它们 [跟踪配置](#tracking-configuration).
...
>[!NOTE]
>
>可以更改公共资源存储目录。 有关详细信息，请参见 [管理公共资源](#managing-public-resources).
...
声明和配置备用服务器必须在营销服务器的 **serverConf.xml** 文件。 中所有可用的参数，请参见 **serverConf.xml** 中列出 [部分](../../installation/using/the-server-configuration-file.md).
...
对于每个必须同步的公用资源，您必须将状态属性添加到 `<url>` 中的元素 `<relay>` 部分：
...
* 列入阻止列表黑名单：如果URL返回404错误，则会将其添加到。 阻止列表中的URL的持续时间（以秒为单位）由 **timeout** 默认值为60秒的属性。
