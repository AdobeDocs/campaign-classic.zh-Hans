...
* **[!UICONTROL Reply address text]** :该名称是可自定义的，当收件人单击其电子邮件客户端 **[!UICONTROL Reply]** 软件中的按钮时，将使用该名称。
...
在Adobe Campaign数据库中，必须按如下方式创建所有电子邮件地址：`x@y.z`。 **x**、**y**&#x200B;和&#x200B;**z**&#x200B;字符不得为空，且不得包含未授权的字符。

您可以在此处在数据库的电子邮件字段中定义授权字符（“数据策略”）。 当通过界面、Web窗体以及在导入数据时，在数据库中输入信息时，列表中未包含的字符将被禁止，因此会被拒绝。

提供了两个列表：**仅欧洲**&#x200B;或&#x200B;**仅美国**。 如有必要，可以添加其他字符。

### 投放参数 {#delivery-parameters}

**高级参数……**&#x200B;链接允许您访问投放选项、链接到重试和隔离的参数。
...
只有在部署向导的第一个步骤中选择了&#x200B;**电子邮件渠道**&#x200B;和&#x200B;**退回邮件**&#x200B;管理选项时，此步骤才可用。 请参阅[常规参数](#general-parameters)。
...
* **[!UICONTROL Security]** :如有必要， **[!UICONTROL SSL]** 从下拉列表中选择
...
只有在部署向导的第一页中选择了&#x200B;**电子邮件渠道**&#x200B;和&#x200B;**跟踪**&#x200B;选项时，才提供此步骤。 请参阅[常规参数](#general-parameters)。
...
1. **在现有实例中添加对跟踪的支持**
   ...
   输入重定向服务器上&#x200B;**internal**&#x200B;帐户的密码，以配置跟踪实例。
...
1. **创建专用于跟踪的新实例**
   ...
   指定实例的名称、密码以及任何关联的DNS掩码，如&#x200B;**[!UICONTROL Campaign*]**。

1. **验证已为您预配置的跟踪实例**

   当您没有&#x200B;**internal**&#x200B;帐户的密码时，会使用此选项；在这种情况下，跟踪服务器上为您预配置了跟踪帐户。 输入重定向服务器的跟踪帐户的密码以验证跟踪实例。
...
有关重试的信息，请参见&#x200B;**重试周期**&#x200B;和&#x200B;**重试次数**&#x200B;字段：当移动设备不可访问时，默认情况下，程序将重试5次，间隔至少15分钟（最长投放期）。 这些值可以根据您的需求进行调整。
...
1. 新的公共资源URL。 有关更多信息，请参阅[公共资源URL](#public-resources-url)一节。
1. 投放中的图像检测模式。 有关更多信息，请参阅[投放图像检测](#delivery-image-detection)一节。
1. 发布选项。 有关更多信息，请参阅[发布模式](#publication-modes)一节。

可通过Adobe Campaign树的&#x200B;**管理>资源>联机>公共资源**&#x200B;节点访问公共资源。 这些量度收集在库中，并可包含在电子邮件中，但也可用于营销活动或任务，以及内容管理中。
...
字段&#x200B;**URL掩码**&#x200B;允许您指定自动上传图像时要跳过的URL掩码列表。 例如，如果您使用的图像存储在可从外部访问的网站上，特别是在互联网网站上，则可以在此字段中输入网站URL。
...
* 跟踪服务器
   ...
   资源将自动复制到不同的跟踪服务器。 这些配置在步骤[跟踪配置](#tracking-configuration)中进行配置。
...
对于图像，必须将它们从通过&#x200B;**NmsDelivery_ImageSubDirectory**&#x200B;选项指定的“images”文件夹复制到一个或多个前端服务器。 这些服务器将存储图像，以便通过新配置的URL访问它们。

如果在Adobe Campaign服务器上发布时没有手动发布脚本，则默认情况下，投放的图像会存储在`$(XTK_INSTALL_DIR)/var/res/img/ directory`中。 相应的URL如下：**`https://server/res/img`**。
...
必须在营销服务器的&#x200B;**serverConf.xml**&#x200B;文件中声明和配置备用服务器。 **serverConf.xml**&#x200B;中可用的所有参数都列在此[部分](../../installation/using/the-server-configuration-file.md)中。
...
对于每个必须同步的公共资源，必须向`<relay>`部分的`<url>`元素添加状态属性：
