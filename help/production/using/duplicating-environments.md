...
1. 在目标环境的所有实例中恢复这些副本，
1. 在启动目标环境前，运行&#x200B;**nms:freezeInstance.js**&#x200B;烧灼脚本。

   此过程不会影响服务器及其配置。

   >[!NOTE]
   >
   >在Adobe Campaign环境中，**烧灼**&#x200B;组合了允许您停止与外部交互的所有进程的操作：日志、跟踪、投放、活动工作流等。\
...
本节将帮助您了解通过案例研究将源环境传输到目标环境所需的步骤：我们的目标是将生产环境（**prod**&#x200B;实例）恢复为开发环境（**dev**&#x200B;实例），以在尽可能接近“live”平台的上下文中工作。
...
* 在Linux中：运行&#x200B;**ps aux | grep nlserver**&#x200B;命令并检查没有&#x200B;**nlserver**&#x200B;进程。

### 步骤4 —— 在目标环境(dev){#step-4---restore-the-databases-in-the-target-environment--dev-}中恢复数据库
