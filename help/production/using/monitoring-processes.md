...
* 執行個體的資訊：版本、名稱、資料庫引擎、已安裝的套件、伺服器系統指標、
* 遺失的處理程式和執行資訊（開始日期、PID等）的清單，
* 工作流程和傳遞的檢視。

監控Campaign流程的其他方式在中介紹 [此頁面](../../production/using/monitoring-guidelines.md).
...
瀏覽至系統指示器清單，以顯示有關電腦的資訊，例如其實體和虛擬記憶體、作用中的處理序和可用的磁碟空間。 Linux和Windows作業系統的指標不同。 前往 **[!UICONTROL Instance Monitoring]** 頁面，然後按一下 **[!UICONTROL Display]** 開啟指標清單的連結。

#### Windows {#in-windows}

* **[!UICONTROL Pending events queued]**：指標特定於 **訊息中心**. [了解详情](../../message-center/using/additional-configurations.md#monitoring-thresholds)

* **[!UICONTROL Memory]**：實體記憶體(RAM)的相關資訊。
   ...
   **[!UICONTROL Alert]**：當記憶體耗用量達到總數量的90%時，會顯示此指標。
   ...
   當 **[!UICONTROL Warning]** 和 **[!UICONTROL Alert]** 指示器會顯示出來，您可以將RAM新增至安裝Adobe Campaign伺服器的電腦，以解決此問題。 您也可以決定在專屬電腦上安裝Adobe Campaign伺服器。

* **[!UICONTROL Swap Memory]**：與符合分頁檔的虛擬記憶體相關的資訊：Windows在硬碟上使用的區域，就像是RAM一樣。
...
* **[!UICONTROL Number of processes too old]**：有關已啟動超過一天的Adobe Campaign流程的資訊。
   ...
   **[!UICONTROL Max Value]**：授權處理序的最大數量(1)。

   **[!UICONTROL Alert]**：如果處理序數等於1，則顯示此指標。

   當 **[!UICONTROL Alert]** 指示器顯示，可能是相關處理序被SQL資料庫引擎鎖定，或是卡在無限回圈中。 此 **看門狗** Adobe Campaign提供的程式每天自動重新啟動所有程式，並可讓您解決此問題。 不過，您也可以自行停止相關程式，以強制重新啟動。
...
* **[!UICONTROL Load average (1/5/15 minutes)]**：有關負載的資訊，即電腦上執行之處理作業在最近5分鐘或15分鐘內的處理器使用率

   **[!UICONTROL Current value]**：電腦的實際負載。

   **[!UICONTROL Max value]**：電腦上處理序的最大使用負載

   **[!UICONTROL Warning]**：此指標會在最後一分鐘、五分鐘或十五分鐘內的載入達到最大授權值的80%時顯示。

   **[!UICONTROL Alert]**：當負載達到最近1分鐘、5分鐘或15分鐘最大授權值的90%時顯示此指標。
...
* **[!UICONTROL Swap Memory]**：與符合分頁檔的虛擬記憶體相關的資訊：Windows在硬碟上使用的區域，就像是RAM一樣。
   ...
   **[!UICONTROL Alert]**：當記憶體耗用量達到總數量的90%時，會顯示此指標。
   ...
   當 **[!UICONTROL Warning]** 和 **[!UICONTROL Alert]** 指示器會顯示出來，您可以藉由增加exchange檔案的大小來解決問題。
...
* **[!UICONTROL Core Files]**：Adobe Campaign程式當機後所產生檔案的相關資訊。 這些檔案可讓您診斷當機的原因。
   ...
   **[!UICONTROL Alert]**：當檔案數等於1時顯示此指標。

   當處理程式因當機而遺失時，會在處理程式清單中以紅色顯示，並由自動重新啟動 **看門狗** Adobe Campaign提供的程式。

* **[!UICONTROL Number of shared memory segments]**：有關所有Adobe Campaign處理序共用的記憶體區段的資訊。

   **[!UICONTROL Current value]**：目前使用中的記憶體區段數目。

   **[!UICONTROL Max Value]**：授權的記憶體區段數上限(2)。

   **[!UICONTROL Warning]**：當記憶體區段數達到1時，會顯示此指示器。

   **[!UICONTROL Alert]**：當記憶體區段數達到2時，會顯示此指示器。

* **[!UICONTROL Number of processes too old]**：有關已啟動超過一天的程式的資訊。

   **[!UICONTROL Current value]**：目前作用中的處理序數目。

   **[!UICONTROL Max Value]**：已授權程式的最大數量。
...
* **[!UICONTROL File Handles]**：有關檔案描述元的資訊，即每個處理序開啟的檔案數。
   ...
   **[!UICONTROL Current value]**：目前檔案描述元數目。

   **[!UICONTROL Max Value]**：作業系統授權的檔案描述元數上限。

   **[!UICONTROL Warning]**：當授權檔案描述項的數目達到80%臨界值時會顯示此指標。

   **[!UICONTROL Alert]**：當授權檔案描述項的數目達到90%臨界值時會顯示此指標。
...
* **[!UICONTROL Processes]**：關於機器程式的資訊。
   ...
   **[!UICONTROL Active Processes]**：作用中處理序數目。

   **[!UICONTROL Inactive Processes]**：非作用中處理序的數量。

   **[!UICONTROL Warning]**：當授權處理的數量達到80%臨界值時，會顯示此指標。

   **[!UICONTROL Alert]**：當授權處理的數量達到90%臨界值時，會顯示此指標。

* **[!UICONTROL Zombie Processes]**：有關已停止但仍具有處理序識別碼(PID)且顯示在處理序表格中的處理序的資訊。

   **[!UICONTROL Current value]**：目前作用中的殭屍處理序數目。

   **[!UICONTROL Max Value]**：授權殭屍程式的最大數量(2)。

   **[!UICONTROL Warning]**：當殭屍程式數量接近2時，會顯示此指標。
...
1. 建立 **.sh** 檔案並為其命名 **[!UICONTROL cust_indicators.sh]** .
1. 將自訂指標新增至此檔案。 例如：
...
>[!IMPORTANT]
>
>* 與SMTP監視相關的資訊只有在電子郵件通道已啟用時才可用。
>* 此 **[!UICONTROL SMTP sending statistics]** 僅當在執行個體上啟動統計資料伺服器時，才會提供。
>
...
1. 此圖表顯示捨棄的訊息的詳細資訊（第2個圖表的紅色曲線）：它顯示捨棄而未重試的訊息(Mauve)與傳送失敗（紅色）的訊息之間的比例。 這可讓您檢視由於統計資料伺服器的限制（節流）或遠端伺服器無法使用，而在授權期間內未處理的訊息比例。
1. SMTP連線已開啟或正在開啟。
1. 預估數量 **matachild**.
...
* 此 **網域** 欄包含訊息傳送到的網域名稱(或真正的網域名稱，例如yahoo.fr的yahoo.com)，
* 此 **Cnx** 欄會顯示此網域開啟的SMTP連線數目。
* 此 **已傳送** 欄對應於傳送至此網域的訊息數，
* 此 **音量** 欄顯示嘗試傳送至此網域的訊息數量（近似值）。
* 此 **錯誤** 欄會顯示一段時間內此網域上錯誤的音量指示器，
* 此 **上次回應** 資料行顯示此網域收到的最後一個SMTP回應訊息，
* 此 **日期** 欄會顯示此網域收到的最後一個SMTP回應的日期。

>[!NOTE]
>
>值顯示在 **Cnx**， **已傳送**、和 **音量** 欄是依據以下專案中所選期間來計算的： **[!UICONTROL Period]** 欄位。
...
此 **[!UICONTROL Owner of domain]** 欄位可讓您將各種網域名稱群組在同一標籤下。 在初始報告檢視中，所有MX網域名稱都將與此擁有者相關聯。
...
* **[!UICONTROL Start date]** ：傳送的開始日期。 請注意，該日期可能早於報表的「起始」日期。
* **[!UICONTROL Label]** ：傳遞的標籤。 要傳送少於100則訊息的傳遞被視為太小，因此會依開始日期彙總，在這種情況下，標籤會顯示彙總數量，例如 [3個小型傳遞的彙總].
* **[!UICONTROL Total volume]** ：為傳遞傳輸的位元組總數。
* **[!UICONTROL Avg volume]** ：傳輸的平均位元組數量。 這是下列公式的結果 **（總量/訊息）**，此量度的計算基礎為 **[!UICONTROL Multiplier]** 量度。
* **[!UICONTROL Messages]** ：已傳送的訊息數。 這包括成功傳送的訊息和重試（在收到來自連絡伺服器的退回訊息後）。
* **[!UICONTROL Multiplier (x)]** ：乘數的值是從訊息的平均數量推算而得。
...
此 **http(s)：//`<Application server url>`/nl/jsp/ping.jsp**  page的運作方式與其網路對應專案相同：它會測試透過apache/tomcat/web模組/資料庫並上傳至使用者端的完整查詢。 如果一切正常運作，則會傳回「OK」。 我們建議在具有資料庫存取權（例如，mta和調查）的電腦上執行此測試。

**使用狀況**：與操作員登入相關聯的工作階段權杖必須作為引數傳入，才能從遠端登入(請參閱 [透過Adobe Campaign指令碼自動監視](#automatic-monitoring-via-adobe-campaign-scripts))。
...
### 根據部署型別進行監視 {#monitoring-based-on-deployment-types}

Adobe Campaign會啟用各種部署設定(如需詳細資訊，請參閱 [本節](../../installation/using/hosting-models.md))。 本節根據您的安裝型別，詳細說明要套用的各種自動監控技術。
...
* 它必須安裝在具有JRE或JDK的電腦上，
* 在Linux中，要監控的電腦必須具備 **bc** 封裝。 如需详细信息，请参阅[此部分](../../installation/using/installing-packages-with-linux.md#distribution-based-on-rpm--packages)。

### 安裝程式 {#installation-procedure}

安裝程式如下：

1. 在主控台中，視需要建立新運運算元（「監視」使用者已存在），但不指派任何許可權。
1. 執行封存擷取。
1. 閱讀 **讀我檔案** 檔案。
...
>[!NOTE]
>
>您可以將尾碼新增至來指定各種設定。 **netconf.xml** 檔案，例如， **netconf-dev.xml**， **netconf-prod.xml**&#x200B;等。 然後，指定用於執行netreport的設定 **netreport.bat** 或 **netreport.sh** 檔案（透過新增） **$JAVA_HOME/bin/java netreport dev** 或 **@%JAVA_HOME%binjava netreport prod** 例如。
...
* 您可以新增 **`<sla>`** （選用）中的子元素 **屬性** 元素。 每次執行netreport時都會產生記錄檔。 檔案名稱包含設定名稱以及日期和時間，例如 **dev_06_12_13_16_47_05.tmp**. 檔案包含下列資訊：執行個體名稱、電腦名稱、嚴重性層級、（0到3，從最不嚴重到最嚴重）、日期（時間戳記格式）、查詢和回應之間經過的時間（毫秒）、使用的服務(http、ncs、ncsex、redir)。 這項資訊會以表格標籤和每個服務結尾的分行符號分隔。

>[!NOTE]
>
>此 **persistHtmlFile** 具有值「true」的屬性，在 **`<property>`** 元素用於在檔案中記錄最新的監視狀態 **netreport.md**. 此檔案會儲存在安裝目錄中。
...
* **收件者清單** （選用）：可讓您透過電子郵件傳送有關特定執行個體的監控報告。
